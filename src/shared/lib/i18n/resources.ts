// Recursos de traducción i18next (es + en). Extraído de i18n.ts para
// mantener i18n.ts como módulo de init pequeño. Es un blob de datos plano.
export const resources = {
  es: {
    translation: {
      common: {
        save: 'Guardar',
        cancel: 'Cancelar',
        delete: 'Eliminar',
        loading: 'Cargando...',
        back: 'Volver',
        next: 'Siguiente',
        ready: 'Listo',
        close: 'Cerrar',
        confirm: 'Confirmar',
        create: 'Crear',
        offline: 'Sin conexión — los cambios se guardarán al reconectar',
        today: 'Hoy',
        hide: 'Ocultar',
        show: 'Mostrar',
        remove: 'Quitar',
        yesterday: 'Ayer',
        error_toast: 'Algo salió mal',
        error_title: 'Algo salió mal',
        error_description: 'Lo sentimos, ha ocurrido un error inesperado.',
        reload: 'Recargar',
      },
      nav: {
        home: 'Inicio',
        routines: 'Rutinas',
        cardio: 'Cardio',
        // Fue "Stats" mientras la pestaña medía ~78 px y la palabra larga se
        // partía. Ya no: esto solo se usa en la fila del cajón, que tiene el
        // ancho del panel entero, y allí el anglicismo desentonaba entre
        // "Biblioteca de ejercicios", "Mis medidas" y "Guía de uso".
        stats: 'Estadísticas',
        progress: 'Progreso',
        settings: 'Ajustes',
        menu: 'Menú',
        menu_close: 'Cerrar menú',
        group_training: 'Entrenamiento',
        group_more: 'Más',
        streak_days_one: '{{count}} día de racha. Ver estadísticas',
        streak_days_other: '{{count}} días de racha. Ver estadísticas',
      },
      empty: {
        workout_title: 'Empieza a entrenar',
        workout_desc: 'Selecciona un ejercicio, añade tus series y guarda tu primer entrenamiento.',
        history_title: 'Sin historial aún',
        history_desc: 'Guarda entrenamientos y aparecerán aquí para que puedas revisarlos.',
        stats_title: 'Sin estadísticas',
        stats_desc: 'Completa entrenamientos para ver tu progreso y tendencias aquí.',
        routine_title: 'Sin rutina configurada',
        routine_desc: 'Planifica tu semana asignando ejercicios a cada día.',
      },
      tips: {
        start_title: 'Comienza tu historial',
        start_msg:
          'Registra al menos 3 entrenamientos para recibir consejos personalizados basados en tus datos.',
        streak_strong_title: '¡Racha de {{count}} días!',
        streak_strong_msg:
          'Consistencia brutal. Asegúrate de incluir al menos 1-2 días de descanso activo por semana para optimizar la recuperación.',
        streak_good_title: 'Buena racha — {{count}} días',
        streak_good_msg:
          'Vas por buen camino. Mantén el ritmo y prioriza el sueño para maximizar las ganancias.',
        rest_long_title: 'Demasiado tiempo sin entrenar',
        rest_long_msg:
          'Llevas {{count}} días sin registrar un entrenamiento. La consistencia es el factor #1 para el progreso. ¡Vuelve hoy!',
        rest_mid_title: '{{count}} días sin entrenar',
        rest_mid_msg:
          'Es normal descansar, pero si llevas más de 3 días sin entrenar planifica tu próxima sesión cuanto antes.',
        freq_low_title: 'Frecuencia baja',
        freq_low_msg:
          'Solo {{count}} sesiones en los últimos 30 días. Para progresar consistentemente, apunta a 3-4 sesiones semanales.',
        vol_down_title: 'Volumen cayó un {{pct}}%',
        vol_down_msg:
          'El volumen semanal bajó significativamente. Puede ser por un descanso planeado (bien) o falta de consistencia. Revisa tu plan.',
        vol_up_title: 'Volumen subió un {{pct}}%',
        vol_up_msg:
          'Gran aumento de volumen. Asegúrate de que el incremento sea progresivo (máx. 10-15%/semana) para evitar sobreentrenamiento.',
        vol_progress_title: '+{{pct}}% de volumen esta semana',
        vol_progress_msg:
          'Progresión constante de volumen. Sigues acumulando estímulo de entrenamiento de forma correcta.',
        balance_push_title: 'Desequilibrio empuje/tracción',
        balance_push_msg:
          'Estás entrenando más músculo de empuje (pecho/hombros) que de tracción (espalda/bíceps). Esto puede causar problemas posturales. Aumenta el volumen de espalda.',
        balance_pull_title: 'Más tracción que empuje',
        balance_pull_msg:
          'Buen énfasis en espalda y bíceps. Considera equilibrar con ejercicios de pecho y hombros para un desarrollo completo.',
        legs_low_title: 'Poco volumen de pierna',
        legs_low_msg:
          'Las piernas representan menos del 15% de tu volumen. Son el grupo muscular más grande del cuerpo — entrenarlas más mejora hormonas anabólicas y fuerza general.',
        legs_none_title: 'No entrenas piernas',
        legs_none_msg:
          'No hay registro de ejercicios de pierna. Sentadilla, peso muerto y prensa son fundamentales para el progreso global.',
        pr_none_title: 'Sin records recientes',
        pr_none_msg:
          'No has batido ningún record en los últimos 30 días. Prueba a aumentar el peso un 2.5% en tu ejercicio principal o añadir 1 rep más.',
        pr_streak_title: '{{count}} nuevos records recientes',
        pr_streak_msg:
          '¡Estás en racha de PRs! Tu progresión es excelente. Mantén la técnica impecable al aumentar la carga.',
        dur_short_title: 'Sesiones muy cortas',
        dur_short_msg:
          'Promedio de {{count}} minutos. Las sesiones ideales duran 45-75 min para un estímulo óptimo. Añade más ejercicios o series.',
        dur_long_title: 'Sesiones muy largas',
        dur_long_msg:
          'Promedio de {{count}} minutos. Entrenamientos de más de 2 horas pueden aumentar el cortisol. Optimiza el tiempo entre series.',
        variety_low_title: 'Poca variedad de ejercicios',
        variety_low_msg:
          'Solo {{count}} ejercicios distintos. Añade variedad para estimular más fibras musculares y evitar adaptaciones.',
        consistency_title: '{{count}} entrenamientos registrados',
        consistency_msg:
          '¡Llevas mucho tiempo registrando tus entrenos! La consistencia a largo plazo es la clave del éxito.',
      },
      coach: {
        notify_train_title: 'Hoy toca entrenar',
        notify_train_body: '{{muscle}} lleva días sin trabajarse. ¿Le damos hoy?',
        page_title: 'Entrenador',
        page_subtitle: 'Análisis de tus datos con IA',
        generate: 'Analizar mi semana',
        regenerate: 'Volver a analizar',
        analyzing: 'Analizando tus datos...',
        empty_title: 'Sin análisis todavía',
        empty_desc: 'Pulsa el botón para que el entrenador revise tus últimas semanas.',
        disclaimer:
          'El entrenador interpreta tus datos, no te examina. No sustituye a un médico, un fisioterapeuta ni a un entrenador presencial.',
        needs_professional_title: 'Consulta a un profesional',
        needs_professional_body:
          'Por lo que cuentas, esto se sale de lo que puede valorar una app. Que te vea un médico o un fisioterapeuta antes de seguir entrenando esa zona.',
        insights_title: 'Lo que veo',
        suggestions_title: 'Qué haría yo',
        apply: 'Aplicar',
        dismiss: 'Descartar',
        applied: 'Aplicada',
        chat_title: 'Pregúntale',
        chat_empty:
          'Pregúntale lo que quieras sobre tu entrenamiento. Ve tus datos, así que no hace falta que se los cuentes.',
        chat_placeholder: 'Escribe tu pregunta...',
        chat_send: 'Enviar',
        banner_title: 'Sugerencia del entrenador',
        banner_dismiss: 'Cerrar la sugerencia',
        home_hint: 'Revisa tu semana y pregúntale',
        kind: {
          load: 'Carga',
          volume: 'Volumen',
          frequency: 'Frecuencia',
          deload: 'Descarga',
          rest: 'Descanso',
          exercise_swap: 'Cambio de ejercicio',
        },
        error: {
          unauthorized: 'Tu sesión ha caducado. Vuelve a entrar.',
          consent_required: 'Activa el entrenador en Ajustes para usarlo.',
          quota_exceeded: 'Has agotado los análisis de hoy. Vuelve mañana.',
          coach_disabled: 'El entrenador no está disponible ahora mismo.',
          provider_unavailable: 'El servicio va saturado. Prueba en unos minutos.',
          invalid_output: 'La respuesta ha llegado mal. Prueba otra vez.',
          network: 'Sin conexión. Comprueba tu red.',
          unknown: 'Algo ha fallado. Prueba otra vez.',
        },
        consent: {
          title: 'Activar el entrenador IA',
          intro:
            'El entrenador manda un resumen de tus datos de entrenamiento a un servicio de IA externo para darte consejo personalizado. Está apagado hasta que tú lo enciendas.',
          data_title: 'Qué sale de tu dispositivo',
          data_profile: 'Tu objetivo, días por semana y material disponible',
          data_body: 'Franja de edad, sexo y peso redondeado — nunca tu fecha de nacimiento exacta',
          data_training: 'Series, pesos y esfuerzo por ejercicio, ya agregados por semanas',
          data_wearable: 'Si tienes wearable: medias de sueño y pulso en reposo',
          data_never:
            'Nunca salen tu nombre, tu correo, tu identificador ni tus entrenamientos uno a uno.',
          provider_title: 'Quién lo procesa',
          provider_body:
            'Un proveedor de IA externo en su nivel gratuito. Eso significa que no hay un acuerdo de tratamiento de datos como el que tendría un servicio de pago, y que podrían usar las peticiones para mejorar sus servicios.',
          medical_title: 'Lo que no es',
          medical_body:
            'No es consejo médico. No diagnostica, no receta dieta ni suplementos, y no sustituye a un profesional.',
          revoke_title: 'Puedes echarte atrás',
          revoke_body:
            'Al desactivarlo se borran de verdad tu memoria del entrenador, tus conversaciones y sus sugerencias.',
          accept: 'Activar el entrenador',
          cancel: 'Ahora no',
        },
        settings: {
          section: 'Entrenador IA',
          toggle: 'Entrenador IA',
          toggle_desc: 'Consejo personalizado analizando tus datos. Apagado por defecto.',
          open: 'Abrir entrenador',
          memory: 'Lo que sabe de ti',
          memory_desc: 'Revisa y borra lo que el entrenador ha aprendido',
          purge: 'Borrar los datos del entrenador',
          purge_title: '¿Apagar el entrenador?',
          purge_confirm:
            '¿Borrar la memoria, las conversaciones y las sugerencias del entrenador? No se puede deshacer.',
          purge_cta: 'Apagar y borrar',
          purged: 'Datos del entrenador borrados',
        },
        memory: {
          title: 'Lo que sabe de ti',
          desc: 'Lo que el entrenador ha aprendido en vuestras conversaciones. Bórralo cuando quieras.',
          empty: 'Todavía no ha aprendido nada de ti.',
          delete: 'Borrar',
          category: {
            injury: 'Lesiones',
            preference: 'Preferencias',
            constraint: 'Limitaciones',
            goal: 'Objetivos',
          },
        },
        next_session: 'Próxima sesión',
        suggestion_title: 'Sugerencia de carga',
        // "de 100 kg × 8" → "a 102,5 kg × 8"
        from_to: '{{from}} kg × {{fromReps}} → {{to}} kg × {{toReps}}',
        last_label: 'Última',
        next_label: 'Recomendado',
        delta_increase: '+{{deltaKg}} {{unit}} (+{{deltaPct}}%)',
        delta_reduce: '−{{deltaKg}} {{unit}} (−{{deltaPct}}%)',
        delta_hold: '±0 {{unit}}',
        confidence_low: 'Poca evidencia',
        confidence_medium: 'Evidencia media',
        confidence_high: 'Buena evidencia',
        volume_weekly_sets_one: '{{count}} serie esta semana',
        volume_weekly_sets_other: '{{count}} series esta semana',
        action: {
          increase: 'Sube la carga',
          hold: 'Mantén la carga',
          reduce: 'Baja la carga',
        },
        reason: {
          bodyweight_add_set:
            'Llegaste al techo de repeticiones y no hay carga que subir. Suma una serie y vuelve al suelo del rango.',
          bodyweight_add_load:
            'Con estas series ya trabajas más resistencia que fuerza. Toca lastrar: empieza por el disco más pequeño que puedas montar.',
          at_failure:
            'Llegaste al límite y perdiste repeticiones. Baja un 5% y recupera la técnica antes de volver a subir.',
          margin_left:
            'Te sobraron repeticiones en recámara las últimas sesiones. Toca subir carga.',
          too_hard:
            'Llevas dos sesiones apurando por encima de tu objetivo. Consolida este peso antes de seguir subiendo.',
          on_target:
            'Estás en el punto justo de esfuerzo. Mantén el peso e intenta sumar una repetición.',
          add_rep:
            'La carga es demasiado ligera para subir de disco sin pasarte. Progresa sumando repeticiones.',
          ceiling:
            'Alcanzaste el techo de tu rango de repeticiones. Sube un escalón de carga y vuelve a trabajar desde el suelo del rango.',
          no_effort_reps:
            'Sin esfuerzo registrado (RIR/RPE), sigo tu última sesión: mismo peso y una repetición más.',
          no_effort_increase:
            'Sin esfuerzo registrado, pero ya alcanzaste el techo del rango. Sube un escalón de carga.',
          deload:
            'Volumen al alza tres semanas seguidas, cada vez cuesta más y las sesiones se valoran peor. Toca una semana de descarga.',
          no_deload: 'No hay señales de que necesites descargar todavía.',
          finish_the_sets:
            'Solo la primera serie llegó al techo del rango; las demás se quedaron cortas. Consolida el peso hasta completar todas las series antes de subir.',
          stale_data:
            'Hace más de dos semanas que no tocas este ejercicio. Repite el último peso para volver a medir dónde estás antes de subir.',
          weekly_cap:
            'Ya subiste la carga esta misma semana. Dale una sesión más a este peso: dos saltos en siete días no da tiempo a asimilarlos.',
          stall_hold:
            'Llevas varias sesiones sin mejorar tu marca. Subir más peso no va a desatascarlo: repite carga y busca calidad de repetición.',
          stall_reset:
            'El estancamiento se alarga. Baja un 10% y vuelve a subir desde ahí: coger carrerilla desatasca más que insistir.',
          volume_spike:
            'Este músculo lleva bastante más trabajo esta semana que en las últimas cuatro. Mantén la carga hasta que el volumen se asiente.',
          volume_over_mrv:
            'Ya acumulas más series semanales de las que sueles recuperar en este músculo. La carga se queda como está.',
          volume_below_mev:
            'Estás haciendo pocas series semanales de este músculo. Antes de tocar el peso, añade series.',
        },
        readiness: {
          ok: 'Tu recuperación acompaña.',
          low_sleep:
            'Llevas varios días durmiendo poco ({{minutes}} min de media). Mantén la carga en lugar de subirla.',
          high_rhr:
            'Tu frecuencia cardiaca en reposo está por encima de tu media habitual. Mantén la carga hoy.',
          sleep_and_hr:
            'Duermes poco y tu pulso en reposo está alto: tu cuerpo aún no se ha recuperado. Mantén la carga.',
        },
        stall: {
          title: '{{exercise}} lleva {{count}} sesiones sin mejorar',
          cause_fatigue:
            'Entrenas siempre al límite. El problema no es el estímulo, es la recuperación: baja algo la intensidad o descansa más entre sesiones.',
          cause_frequency:
            'Pasan demasiados días entre sesiones. Entrenarlo más a menudo, aunque sea con menos series, suele desatascarlo.',
          cause_volume:
            'Estás haciendo muy pocas series. Añade una o dos series por sesión antes de tocar el peso.',
          cause_unknown:
            'Prueba a cambiar algo: rango de repeticiones, tempo o una variante del ejercicio.',
        },
      },
      /* Nombres de los grupos musculares para MOSTRAR.
         El valor guardado en `exercises.muscle_group` es el literal español
         («Bíceps»), lo pone el trigger `autoclassify_muscle_group` y no se
         traduce: lo que se traduce es la etiqueta, en `muscleGroupLabel`. */
      muscleGroups: {
        pecho: 'Pecho',
        espalda: 'Espalda',
        hombro: 'Hombro',
        pierna: 'Pierna',
        gluteo: 'Glúteo',
        biceps: 'Bíceps',
        triceps: 'Tríceps',
        antebrazo: 'Antebrazo',
        core: 'Core',
        cardio: 'Cardio',
        otro: 'Otro',
      },
      /* Equipamiento de los ejercicios propios. Mismo trato que muscleGroups:
         el valor guardado es el español y aquí solo vive la etiqueta. */
      equipment: {
        barra: 'Barra',
        mancuernas: 'Mancuernas',
        maquina: 'Máquina',
        polea: 'Polea',
        peso_corporal: 'Peso corporal',
        bandas: 'Bandas',
        kettlebell: 'Kettlebell',
        otro: 'Otro',
      },
      muscleMap: {
        title: 'Mapa muscular',
        front: 'Frente',
        back: 'Espalda',
        legend_recovering: 'En recuperación',
        legend_partial: 'Casi listo',
        legend_recovered: 'Recuperado',
        legend_untrained: 'Sin entrenar',
        untrained_list: 'Sin datos todavía: {{groups}}',
      },
      userStats: {
        page_title: 'Mis Estadísticas',
        page_subtitle: 'Análisis completo de tu progreso',
        what_now: 'Qué hacer ahora',
        global_summary: 'Resumen global',
        cardio_total_time: 'Tiempo total de cardio',
        recovery_rested: 'Recuperado',
        recovery_recovering: 'En recuperación',
        recovery_partial: 'Casi listo',
        days_ago: 'hace {{count}}d',
        no_data_label: 'Sin datos',
        personalized_tips: 'Consejos personalizados',
        tips_count_one: '{{count}} consejo basado en tus datos',
        tips_count_other: '{{count}} consejos basados en tus datos',
        empty_title: 'Sin datos todavía',
        empty_desc: 'Registra tus primeros entrenamientos para ver tus estadísticas aquí.',
        calendar_title: 'Registro de entrenos',
        calendar_trained_one: '1 día entrenado',
        calendar_trained_other: '{{count}} días entrenados',
        weekday_mon: 'L',
        weekday_tue: 'M',
        weekday_wed: 'X',
        weekday_thu: 'J',
        weekday_fri: 'V',
        weekday_sat: 'S',
        weekday_sun: 'D',
        total_workouts: 'Entrenamientos totales',
        total_volume: 'Volumen total',
        total_prs: 'Records personales',
        streak_days_one: '{{count}} día de racha',
        streak_days_other: '{{count}} días de racha',
        streak_max_suffix: ' (máx. {{max}})',
        sessions_30d: 'sesiones/30d',
        avg_duration: 'duración media',
        distinct_exercises: 'ejercicios distintos',
        volume_pct: '{{pct}}% volumen',
        recovery_title: 'Estado de recuperación',
        recovery_card_title: 'Recuperación',
        muscle_recovered: 'Grupo muscular recuperado',
        suggestion_today: 'Sugerencia para hoy',
        train_muscle: 'Entrenar {{muscle}}',
        days_without_training: 'días sin entrenar',
        days_since_full: 'Hace {{count}} días',
        day_frequency_title: 'Consistencia por día',
        muscle_balance_title: 'Balance muscular',
        muscle_distribution_label: 'Distribución por grupo muscular',
        top_exercises_title: 'Top ejercicios por volumen',
        weekly_volume_title: 'Evolución del volumen',
        last_8_weeks: 'Últimas 8 semanas',
        favorite_day: 'Tu día favorito:',
      },
      onboarding: {
        title: '¡Bienvenido a GymLog v2!',
        subtitle: 'Configuremos tu perfil para darte una mejor experiencia.',
        goal: '¿Cuál es tu objetivo?',
        goal_volume: 'Ganar volumen',
        goal_strength: 'Ganar fuerza',
        goal_endurance: 'Resistencia',
        goal_fat_loss: 'Perder grasa',
        days: '¿Cuántos días entrenas por semana?',
        equipment: '¿De qué equipo dispones?',
        finish: 'Empezar a entrenar',
        sex: '¿Cuál es tu sexo?',
        sex_male: 'Hombre',
        sex_female: 'Mujer',
        sex_other: 'Prefiero no decirlo',
        body: 'Tus medidas',
        height: 'Altura',
        weight: 'Peso',
        body_hint:
          'Se usan para estimar el volumen en los ejercicios de peso corporal. Puedes cambiarlas cuando quieras en Mis medidas.',
      },
      notifications: {
        title: 'Notificaciones',
        clear: 'Borrar todo',
        empty_title: 'Sin notificaciones',
        empty_desc:
          'Aquí aparecerán los avisos de fin de descanso, la rutina del día y tus récords.',
        just_now: 'ahora mismo',
        minutes_ago: 'hace {{count}} min',
        hours_ago: 'hace {{count}} h',
        days_ago: 'hace {{count}} d',
        today: 'Hoy',
        yesterday: 'Ayer',
        delete_one: 'Borrar este aviso',
        filter_all: 'Todo',
        filter_routine: 'Rutina',
        filter_streak: 'Racha',
        filter_summary: 'Resumen',
        filter_pr: 'Récords',
        empty_filter: 'Ningún aviso de este tipo',
      },
      search: {
        placeholder: 'Buscar ejercicio…',
        clear: 'Limpiar búsqueda',
        empty: 'Ningún ejercicio coincide',
        train: 'Entrenar',
        favorites: 'Favoritos',
        favorite_toggle: 'Marcar «{{name}}» como favorito',
        open_library: 'Abrir la biblioteca',
      },
      guide: {
        title: 'Guía de uso',
        subtitle: 'Lo esencial para entender GymLog en 2 minutos.',
        quickstart: 'Tu primer entreno en 3 pasos',
        step1_title: 'Elige un ejercicio',
        step1_desc:
          'En la pestaña Entrenar abre el selector y busca el ejercicio. Con «+ Personalizado» creas los tuyos.',
        step2_title: 'Registra tus series',
        step2_desc: 'Escribe reps y kg y pulsa «+ Serie». Repite tantas veces como series hagas.',
        step3_title: 'Guarda la sesión',
        step3_desc:
          'Pulsa Guardar: el entreno pasa a tu historial y actualiza récords, volumen y racha.',
        sections_title: 'Qué hace cada pantalla',
        tips_title: 'Trucos que conviene saber',
        open: 'Abrir',
        finish: 'Entendido, ¡a entrenar!',
        tips: [
          'En el historial, desliza un entreno hacia un lado para borrarlo.',
          'Con «Repetir último entreno» cargas la sesión anterior de una sola vez.',
          'Puedes entrenar sin conexión: nada se pierde, se sube cuando vuelva la red.',
          'Todo es reversible: si te equivocas en una serie, edítala o bórrala antes de guardar.',
        ],
        workout: {
          title: 'Entrenar',
          bullets: [
            'Selecciona el ejercicio del desplegable, o créalo tú con músculos secundarios y opción de peso corporal.',
            'Introduce reps y kg, pulsa «+ Serie» y ve sumando. «× Todas» limpia la lista.',
            'Cada serie admite RPE (esfuerzo percibido) y tipo: normal, drop set, rest-pause o AMRAP.',
            'Antes de guardar puedes añadir notas de la sesión y una valoración de cómo te has sentido.',
            'Si sales a medias, al volver la app te ofrece continuar o descartar la sesión.',
          ],
        },
        tools: {
          title: 'Ayudas dentro de la serie',
          bullets: [
            'Calculadora de discos: dile el peso objetivo y te dice qué poner a cada lado de la barra.',
            'Ves tu 1RM estimado y tus récords por rango de repeticiones en cada ejercicio.',
            '«Última sesión» muestra lo que hiciste la vez anterior: puedes copiarlo o aplicar la sugerencia de progresión.',
            'En ejercicios de peso corporal, los kg que escribas cuentan como lastre añadido.',
          ],
        },
        rest: {
          title: 'Descanso y avisos',
          bullets: [
            'El temporizador puede arrancar solo al añadir una serie (Ajustes → Entrenamiento).',
            'Puedes fijar una duración fija o dejar que sea mayor en los ejercicios compuestos.',
            'El aviso suena aunque estés en otra pantalla o con la app en segundo plano.',
            'En Android, activa «alarmas exactas» en Ajustes para que el aviso no llegue tarde.',
          ],
        },
        routines: {
          title: 'Rutinas',
          bullets: [
            'Organiza tu semana asignando ejercicios a cada día.',
            'Activa los recordatorios de entreno y la app te avisa el día que toca.',
          ],
        },
        cardio: {
          title: 'Cardio',
          bullets: [
            'Cronómetro para carrera, bici, elíptica y demás sesiones de cardio.',
            'La sesión sigue corriendo aunque cambies de pestaña; lo verás marcado en el icono de Cardio.',
          ],
        },
        history: {
          title: 'Historial',
          bullets: [
            'Todos tus entrenos guardados, con sus series y su volumen.',
            'Entra en cualquier sesión para revisarla, o deslízala para borrarla.',
          ],
        },
        stats: {
          title: 'Estadísticas',
          bullets: [
            'Volumen por semana y por grupo muscular, récords personales y racha de días.',
            'Mapa de consistencia y fatiga muscular: qué grupos están recuperados y cuáles no.',
            'Consejos personalizados según tu objetivo y tus últimas sesiones.',
          ],
        },
        measurements: {
          title: 'Mis medidas',
          bullets: [
            'Registra peso corporal y medidas para seguir su evolución.',
            'El peso corporal se usa para estimar el volumen en ejercicios de peso corporal.',
          ],
        },
        wearables: {
          title: 'Wearables',
          bullets: [
            'Conecta Health Connect para traer pasos, calorías, pulso y sueño desde tu reloj.',
            'Solo funciona en la app de Android, no en la versión web.',
          ],
        },
        offline: {
          title: 'Sin conexión',
          bullets: [
            'Si no hay cobertura, el entreno se guarda en el móvil y no se pierde.',
            'Al recuperar la red se sincroniza solo; verás cuántos entrenos quedan pendientes.',
          ],
        },
        settings: {
          title: 'Ajustes',
          bullets: [
            'Tema oscuro o claro, idioma, unidades (kg/lb) y sonido.',
            'Notificaciones, descanso automático, series de calentamiento y acceso biométrico.',
          ],
        },
      },
      settings: {
        title: 'Ajustes',
        theme: 'Tema',
        theme_system: 'Sistema',
        theme_dark: 'Oscuro',
        accent: 'Color de acento',
        accent_desc: 'Se aplica a botones, iconos y resaltados',
        accent_yellow: 'Amarillo',
        accent_lime: 'Lima',
        accent_mint: 'Menta',
        accent_cyan: 'Cian',
        accent_blue: 'Azul',
        accent_violet: 'Violeta',
        accent_pink: 'Rosa',
        accent_orange: 'Naranja',
        accent_red: 'Rojo',
        accent_amber: 'Ámbar',
        accent_green: 'Verde',
        accent_teal: 'Turquesa',
        accent_indigo: 'Índigo',
        accent_magenta: 'Magenta',
        accent_rose: 'Frambuesa',
        accent_sand: 'Arena',
        accent_slate: 'Pizarra',
        accent_emerald: 'Esmeralda',
        accent_azure: 'Azur',
        accent_lavender: 'Lavanda',
        accent_plum: 'Ciruela',
        accent_terracotta: 'Terracota',
        accent_sage: 'Salvia',
        accent_steel: 'Acero',
        accent_more: 'Ver todos los colores',
        accent_less: 'Ocultar colores',
        app_icon: 'Icono de la app',
        app_icon_desc:
          'Al cambiarlo, Android saca el icono de la pantalla de inicio y lo deja al final del cajón de aplicaciones. Tendrás que recolocarlo.',
        app_icon_match: 'Igual que el color de acento',
        app_icon_changed: 'Icono cambiado. Búscalo en el cajón de aplicaciones.',
        app_icon_error: 'No se ha podido cambiar el icono',
        theme_light: 'Claro',
        language: 'Idioma',
        notifications: 'Notificaciones',
        notifications_desc: 'Permisos para recordatorios',
        exact_alarms: 'Alarmas exactas',
        exact_alarms_desc: 'Sin este permiso el aviso de descanso puede llegar tarde',
        exact_alarms_action: 'Activar',
        exact_alarms_on: 'Alarmas exactas activadas',
        sound: 'Sonido',
        sound_desc: 'Sonido al completar series',
        logout: 'Cerrar sesión',
        version: 'GymLog v{{version}}',
        about: 'Acerca de',
        rest_duration: 'Duración del descanso',
        rest_duration_desc: 'Tiempo del temporizador automático',
        rest_by_exercise: 'Descanso según ejercicio',
        rest_by_exercise_desc: 'Más descanso en ejercicios compuestos (≈2×)',
        auto_fill_weights: 'Auto-rellenar pesos',
        auto_fill_weights_desc: 'Precargar el peso de la última sesión al empezar una rutina',
        keep_screen_awake: 'Pantalla siempre encendida',
        keep_screen_awake_desc:
          'Evita que el móvil se bloquee mientras entrenas. Se suelta al terminar la sesión',
        save_scope: 'Guardado de series',
        save_scope_desc: 'Qué hacer al guardar si tienes series completadas y otras sin marcar',
        save_scope_ask: 'Preguntar',
        save_scope_all: 'Guardar todas',
        save_scope_completed: 'Solo completadas',
        wearables: 'Dispositivos / Wearables',
        wearables_desc: 'Conecta Amazfit, Garmin y más',
        preferences: 'Preferencias',
        training: 'Entrenamiento',
        data: 'Datos',
        account: 'Cuenta',
        my_measurements: 'Mis medidas',
        google_account: 'Cuenta de Google',
        change_photo: 'Cambiar foto de perfil',
        photo_updated: 'Foto de perfil actualizada',
        photo_error: 'No se pudo subir la foto',
        photo_too_large: 'Imagen demasiado grande (máx. 2 MB)',
        edit_name: 'Editar nombre',
        name_placeholder: 'Tu nombre',
        name_updated: 'Nombre actualizado',
        name_error: 'No se pudo guardar el nombre',
        biometric: 'Acceso biométrico',
        biometric_desc: 'Usa tu huella o cara para entrar',
        biometric_unavailable: 'Biometría no disponible: {{message}}',
        biometric_verifying: 'Verificando identidad...',
        biometric_on: 'Acceso biométrico activado',
        biometric_off: 'Acceso biométrico desactivado',
        biometric_failed: 'Autenticación fallida',
        biometric_sensor_error: 'Error al conectar con el sensor',
        biometric_locked_title: 'GymLog está bloqueado',
        biometric_locked_hint: 'Usa tu huella o cara para acceder',
        biometric_unlock: 'Desbloquear',
        biometric_retry: 'Reintentar',
        training_reminders: 'Recordatorios de entreno',
        training_reminders_desc: 'Avisar de la rutina del día y de la racha en peligro',
        reminder_times: 'Horas de los avisos',
        reminder_routine: 'Rutina del día',
        reminder_streak: 'Racha en peligro',
        reminder_summary: 'Resumen semanal',
        reminder_summary_day: 'Día del resumen',
        quiet_hours: 'No molestar',
        quiet_hours_desc: 'Dentro de este rango los avisos se guardan pero no suenan',
        quiet_hours_from: 'Desde',
        quiet_hours_to: 'Hasta',
        quiet_conflict: 'Este aviso cae dentro del rango de silencio: se guardará sin sonar',
        notifications_blocked: 'Android tiene bloqueadas las notificaciones de GymLog',
        notifications_blocked_action:
          'Actívalas en Ajustes de Android → Aplicaciones → GymLog → Notificaciones',
        weight_unit: 'Unidad de peso',
        warmup_sets: 'Series de calentamiento',
        warmup_sets_desc: 'Marcar series de warmup',
        rest_autostart: 'Auto-iniciar descanso',
        rest_autostart_desc: 'Inicia el temporizador al añadir serie',
        download_apk_title: 'Descargar App Android',
        tagline: 'Tu compañero definitivo para el seguimiento de entrenamientos de fuerza.',
        notif_on: 'Notificaciones activadas',
        notif_denied: 'No se concedieron permisos de notificación',
        notif_denied_browser: 'Permiso de notificaciones denegado en el navegador',
      },
      wearables: {
        title: 'Dispositivos',
        disconnect: 'Desconectar',
        sync_now: 'Sincronizar ahora',
        syncing: 'Sincronizando…',
        connected: 'Conectado',
        disconnected: 'No conectado',
        last_sync: 'Última sincronización: {{when}}',
        never_synced: 'Nunca sincronizado',
        sync_on_open: 'Sincronizar al abrir',
        sync_on_open_desc: 'Trae datos automáticamente al abrir la app',
        health_aggregator: 'Salud del teléfono',
        health_aggregator_desc_android: 'Health Connect: Amazfit, Samsung, Garmin y más',
        health_aggregator_desc_ios: 'Apple Salud: Amazfit, Apple Watch y más',
        amazfit_hint: 'Para Amazfit: activa la sincronización de la app Zepp hacia {{target}}.',
        grant_permission: 'Conceder permisos',
        permission_needed: 'Concede permisos de salud para sincronizar',
        steps: 'Pasos',
        resting_hr: 'FC en reposo',
        distance: 'Distancia',
        calories: 'Calorías',
        hr_avg_max: 'FC med/máx',
        sleep: 'Sueño',
        sleep_duration: 'Duración del sueño',
        sync_ok: 'Sincronizado: {{daily}} días, {{sleep}} noches, {{workouts}} entrenos',
        sync_error: 'Error al sincronizar',
        not_available_web: 'Solo disponible en la app móvil',
        no_data: 'Sin datos todavía. Sincroniza para empezar.',
        bpm: 'ppm',
        strength_imported_title: 'Sesiones de gimnasio',
        strength_imported_desc:
          'Se importaron {{count}} sesiones de gimnasio con su tiempo, calorías y frecuencia cardíaca. No traen series ni pesos: esos los registras tú en la app, y aparecen juntos en el historial.',
      },
      workout: {
        title: 'Entrenar',
        select_exercise: '- Ejercicio -',
        custom_exercise: '+ Personalizado',
        notes: 'Notas',
        no_notes: 'Anotaciones',
        new_note: 'Nueva nota...',
        recent_pr: 'PR reciente',
        pr_by_band: 'Récords por rango de repeticiones',
        sets: 'Series',
        volume: 'Volumen',
        active_session: 'Sesión activa',
        repeat_last: 'Repetir último entreno',
        start_cta: 'Empezar a entrenar',
        cancel_session: 'Cancelar entrenamiento',
        cancel_confirm: '¿Cancelar y eliminar la sesión de entrenamiento actual?',
        // El título pregunta y el cuerpo dice qué se pierde: con el diálogo del
        // sistema todo iba en una sola línea y no quedaba claro que se borra.
        cancel_confirm_title: '¿Descartar este entrenamiento?',
        cancel_confirm_body_one: 'Se borrará la serie que llevas registrada. No se puede deshacer.',
        cancel_confirm_body_other:
          'Se borrarán las {{count}} series que llevas registradas. No se puede deshacer.',
        cancel_confirm_accept: 'Descartar',
        cancel_confirm_keep: 'Seguir entrenando',
        reps: 'Reps',
        seconds: 'Segundos',
        mode_reps: 'Reps',
        mode_time: 'Tiempo',
        timer_start: 'Empezar',
        timer_pause: 'Pausa',
        timer_reset: 'Reiniciar',
        timer_accept: 'Aceptar el tiempo',
        timer_elapsed: '{{seconds}} segundos',
        timer_target: 'Objetivo {{time}}',
        weight: 'Kg',
        add_set: '+ Serie',
        complete_set: 'Completar serie',
        previous_set: 'Anterior',
        save_pending_title: 'Guardar entrenamiento',
        save_pending_body:
          'Tienes {{pending}} series sin marcar y {{completed}} completadas. ¿Guardar todas o solo las completadas?',
        save_all: 'Guardar todas',
        save_completed_only: 'Solo completadas',
        set_n: 'Serie {{n}}',
        edit_set: 'Editar serie',
        warmup: 'Calent.',
        set_note_placeholder: 'Nota de la serie...',
        remove_all: '× Todas',
        rest: 'Descanso',
        rest_auto_on: 'Auto on',
        rest_auto_off: 'Auto off',
        rest_remaining: 'Descanso restante',
        rest_restart: 'Reiniciar descanso',
        rest_over: '¡Descanso terminado!',
        stop_alarm: 'Detener',
        save_workout: 'Guardar',
        saving: 'Guardando...',
        session_notes: 'Notas de la sesión',
        session_notes_placeholder: '¿Cómo te has sentido? Sueño, energía...',
        session_rating: 'Valoración',
        empty_sets: 'Añade una serie',
        empty_hint: 'Selecciona un ejercicio y registra tus series.',
        new_pr_title: 'Nuevo record personal',
        new_pr_body: '{{exercise}}: {{weight}} kg estimado de 1RM',
        resume_banner: 'Entrenamiento en curso detectado',
        resume_desc: 'Tienes una sesión iniciada. ¿Quieres continuar?',
        continue: 'Continuar',
        discard: 'Descartar',
        add_valid_set: 'Añade al menos una serie válida',
        errors: {
          needs_measure: 'Añade repeticiones o segundos',
          weight_negative: 'El peso no puede ser negativo',
          invalid_set: 'Serie incompleta',
          pick_exercise: 'Elige un ejercicio antes de guardar',
          no_valid_sets: 'Añade reps y kg válidas',
          sets_incomplete_one: 'La serie {{numbers}} está sin terminar',
          sets_incomplete_other: 'Las series {{numbers}} están sin terminar',
        },
        saved: '✓ Entrenamiento guardado',
        saved_offline: '✓ Guardado sin conexión · se sincronizará',
        pending_sync_one: '1 entreno pendiente de sincronizar',
        pending_sync_other: '{{count}} entrenos pendientes de sincronizar',
        note_save_error: 'No se pudo guardar la nota',
        confirm_delete_exercise: '¿Eliminar este ejercicio? Esta acción no se puede deshacer.',
        delete_exercise_title: '¿Eliminar «{{name}}»?',
        delete_exercise_body:
          'Desaparece de tu lista y se pierden sus records personales. No se puede deshacer.',
        delete_exercise_in_use:
          'No se puede eliminar: tienes entrenos guardados con este ejercicio.',
        delete_exercise_failed: 'No se pudo eliminar el ejercicio.',
        delete_exercise_done: 'Ejercicio eliminado',
        copy_replace_title: '¿Sustituir las series que llevas?',
        copy_replace_body:
          'Copiar la última sesión reemplaza lo que has anotado en este ejercicio. No se puede deshacer.',
        copy_replace_accept: 'Sustituir',
        custom_badge: 'Propio',
        create_custom_exercise: 'Crear ejercicio personalizado',
        new_exercise: 'Nuevo ejercicio',
        exercise_name_placeholder: 'Nombre del ejercicio',
        secondary_muscles: 'Músculos secundarios',
        /* Cabeceras del desplegable de ejercicios. «Recientes» es un montón,
           no un grupo muscular, así que tiene clave propia. */
        group_recent: 'Recientes',
        group_suggested: '{{group}} — Sugerido',
        bodyweight_exercise: 'Ejercicio de peso corporal',
        name_required: 'El nombre es requerido',
        bodyweight_hint: 'Peso corporal: {{weight}}. El kg que introduzcas se suma como lastre.',
        bodyweight_no_weight:
          'Registra tu peso (arriba) para estimar el volumen. Puedes guardar solo con reps.',
        load_label: 'Lastre',
        load_type_label: 'Cómo se realiza',
        load_type_prompt: '¿Cómo haces «{{name}}»?',
        load_type_external: 'Externo',
        load_type_bodyweight: 'Peso corporal',
        load_type_bodyweight_loaded: 'Con lastre',
        rpe: 'RPE',
        rpe_label: 'Esfuerzo percibido (RPE)',
        rpe_optional: '· opcional',
        rpe_option: 'RPE {{value}}',
        rpe_help:
          'Del 6 (muy fácil) al 10 (no podías con una más). Marcarlo es lo que permite ajustar tu carga y avisarte de cuándo descargar.',
        context_hide: 'Ocultar el contexto',
        context_show: 'Ver la última sesión',
        set_type_label: 'Tipo de serie',
        set_type_normal: 'Normal',
        set_type_dropset: 'Drop set',
        set_type_rest_pause: 'Rest-pause',
        set_type_amrap: 'AMRAP',
        plates_calc: 'Calculadora de discos',
        plates_calc_short: 'Calc. discos',
        rate_session: 'Valorar sesión',
        change: 'Cambiar',
        set_note: 'Nota serie',
        plates_target: 'Peso objetivo',
        plates_bar: 'Barra',
        plates_each_side: 'A cada lado de la barra',
        plates_empty: 'Solo la barra',
        plates_leftover: 'No exacto: sobran {{kg}} kg por lado',
        plates_available: 'Discos de mi gimnasio',
        plates_available_none: 'ninguno',
        plates_available_help: 'Marca los discos que tienes. Se recuerdan para la próxima vez.',
        equipment_steps: 'Saltos de mancuerna y máquina',
        equipment_steps_help:
          'Los discos solo describen la barra. Estos son los saltos mínimos del resto del material, y con ellos la sugerencia de carga deja de proponer pesos que no puedes montar.',
        dumbbell_step: 'Mancuerna (kg)',
        machine_step: 'Máquina (kg)',
        e1rm: '1RM estimado',
        last_session: 'Última sesión',
        copy: 'Copiar',
        progression_hint: 'Sugerencia',
        apply: 'Aplicar',
        complete_title: 'Entreno guardado',
        complete_minutes: '{{count}} min',
        complete_sets: '{{count}} series',
      },
      cardio: {
        no_records: 'Sin registros',
        recording: 'Grabando',
        paused: 'En pausa',
        in_progress: 'En curso',
        resume: 'Continuar',
        pause: 'Pausar',
        finish: 'Terminar',
        add_details: 'Añade detalles opcionales',
        distance_km: 'Distancia (km)',
        calories: 'Calorías',
        notes_placeholder: 'Notas (opcional)...',
        discard: 'Descartar',
        save_session: 'Guardar sesión',
        this_week: 'Esta semana',
        sessions: 'Sesiones',
        time: 'Tiempo',
        distance: 'Distancia',
        recent_activity: 'Actividad reciente',
        pick_activity: 'Empezar actividad',
        pick_activity_help: 'Toca una y arranca el cronómetro',
        start_type: 'Empezar {{type}}',
        start_session: 'Iniciar sesión',
        first_session: 'Inicia tu primera sesión de cardio',
        health_source: 'Salud',
      },
      routine: {
        title: 'Rutinas',
        select: 'Selecciona una rutina',
        change: 'Cambiar',
        today: 'HOY',
        predefined: 'Predefinida',
        use_template: 'Usar como plantilla',
        cloned: 'Plantilla copiada — ya puedes editarla',
        create_custom: '+ Crear rutina personalizada',
        share: 'Compartir rutina',
        share_dialog_title: 'Compartir rutina',
        share_error: 'No se ha podido compartir la rutina.',
        print: 'Imprimir o guardar en PDF',
        print_dialog_title: 'Abrir la hoja de la rutina',
        print_error: 'No se ha podido preparar la hoja.',
        print_popup_blocked:
          'El navegador ha bloqueado la ventana. Permite las ventanas emergentes y vuelve a intentarlo.',
        import_file: 'Importar rutina de un fichero',
        import_success: 'Rutina «{{name}}» añadida.',
        import_error: 'No se ha podido leer la rutina.',
        new_routine: 'Nueva Rutina',
        name_placeholder: 'Nombre',
        desc_placeholder: 'Descripción (opcional)',
        custom_default_desc: 'Rutina personalizada',
        delete_routine: 'Eliminar rutina',
        delete_confirm: '¿Eliminar esta rutina?',
        delete_confirm_desc:
          'Se borran la rutina y sus días. Los entrenos que ya guardaste no se tocan.',
        empty_custom_day: 'Este día está vacío',
        empty_custom_day_desc: 'Añade los ejercicios que quieras hacer este día.',
        rest_day: 'Día de descanso',
        rest_day_desc: 'Hoy no toca. Descansar también es parte del plan.',
        starts_on: 'Empieza el {{date}}',
        schedule_short: 'Programar',
        schedule_action: 'Programar cuándo empieza {{name}}',
        hide_short: 'Ocultar',
        hide_action: 'Ocultar la rutina {{name}}',
        unhide_short: 'Mostrar',
        unhide_action: 'Volver a mostrar la rutina {{name}}',
        hidden_show_one: 'Ver 1 rutina oculta',
        hidden_show_other: 'Ver {{count}} rutinas ocultas',
        hidden_collapse: 'Ver menos',
        hidden_hint: 'Las rutinas ocultas no se borran: siguen aquí por si las quieres recuperar.',
        keep_current: 'Seguir con la rutina de ahora',
        hidden_done: '{{name}} ya no aparece en la lista',
        hidden_undo: 'Deshacer',
        hidden_scheduled: 'Programada',
        schedule_title: 'Programar esta rutina',
        schedule_help:
          'El día que elijas, esta rutina pasa a ser la activa sola. Si no abres la app ese día, entra la próxima vez que la abras.',
        schedule_date_label: 'Empieza el',
        schedule_saved: 'Programada para el {{date}}',
        schedule_cleared: 'Fecha quitada',
        schedule_clear: 'Quitar fecha',
        schedule_applied: 'Toca bloque nuevo: {{name}}',
        next_scheduled: 'El {{date}} entra «{{name}}»',
        move_day: 'Mover',
        move_title: 'Mover {{name}} a…',
        move_help: 'Solo cambia esta semana. El lunes vuelve tu rutina de siempre.',
        move_free: 'Libre',
        move_done: 'Movido al {{day}}',
        week_reorganized: 'Semana reorganizada',
        week_reorganized_desc: 'Los cambios valen solo para esta semana.',
        week_restore: 'Restaurar semana',
        week_restored: 'Semana restaurada',
        moved_from: 'Movido desde el {{day}}',
        day_freed: 'Día libre',
        day_freed_desc: 'Moviste este entreno a otro día de esta semana.',
        add_exercise: '+ Añadir',
        add_exercise_full: 'Añadir ejercicio',
        edit_exercise: 'Editar {{name}}',
        exercise_mode: 'Cómo se hace',
        mode_reps: 'Repeticiones',
        mode_time: 'Tiempo',
        field_sets: 'Series',
        field_reps: 'Reps',
        field_reps_placeholder: '10-12',
        field_duration: 'Segundos',
        per_side: 'Por lado',
        per_side_help: 'El objetivo es por cada lado: «3 × 12 por lado» son 24 repeticiones.',
        target_per_side: 'por lado',
        superset: 'Superserie',
        superset_help: 'Va encadenado con «{{name}}», sin descanso entre los dos.',
        superset_label: 'Superserie',
        save_failed: 'No se pudo guardar la rutina. Inténtalo de nuevo.',
        start_session: 'Realizar esta rutina',
        session_in_progress: 'Rutina en curso',
        session_target: 'Objetivo',
        session_finish: 'Completar rutina',
        session_discard: 'Descartar',
        session_discard_confirm: '¿Descartar la rutina en curso? Se perderán las series.',
        session_saving: 'Guardando…',
        session_saved: 'Rutina registrada en tu historial',
        session_saved_offline: 'Rutina guardada — se sincronizará al recuperar conexión',
        days: {
          monday: 'Lunes',
          tuesday: 'Martes',
          wednesday: 'Miércoles',
          thursday: 'Jueves',
          friday: 'Viernes',
          saturday: 'Sábado',
          sunday: 'Domingo',
        },
        session_dropped_sets_one: 'Una serie se quedó a medias y no se ha guardado',
        session_dropped_sets_other: '{{count}} series se quedaron a medias y no se han guardado',
        errors: {
          session_no_sets: 'Registra al menos una serie con reps y kg',
        },
        session_empty: 'Registra al menos una serie con reps y kg',
        session_how_to: 'Cómo hacerlo',
        session_recommended_weight: 'Peso recomendado',
        session_no_recommendation:
          'Aún no hay historial para recomendar un peso. Escribe tú las series y la próxima vez te propongo carga.',
        session_no_advice:
          'No hay ninguna serie con peso ni tiempo. Rellena al menos una antes de terminar.',
        session_log: 'Lo que has hecho',
        session_add_set: 'Añadir serie',
        session_remove_set: 'Quitar la serie {{n}}',
        session_reps_of_set: 'Repeticiones de la serie {{n}}',
        session_weight_of_set: 'Peso de la serie {{n}}',
      },
      measurements: {
        title: 'Medidas corporales',
        weight: 'Peso',
        body_fat: 'Grasa',
        add: 'Añadir medida',
        empty: 'Sin medidas registradas',
        save_error: 'No se pudo guardar la medida',
      },
      weight_prompt: {
        title: '¿Cuánto pesas esta semana?',
        subtitle: 'Lo usamos para estimar el volumen de los ejercicios de peso corporal.',
        placeholder: 'kg',
        save: 'Guardar',
        invalid: 'Introduce un peso entre 1 y {{max}} kg',
      },
      library: {
        title: 'Biblioteca de ejercicios',
        open: 'Biblioteca',
        search: 'Buscar ejercicio…',
        empty: 'Sin ejercicios',
        muscle: 'Músculo',
        equipment: 'Equipo',
        movement: 'Movimiento',
        compound: 'Compuesto',
        no_description: 'Sin instrucciones todavía',
        all: 'Todos',
        all_equipment: 'Todo el material',
        tab_own: 'Mis ejercicios',
        tab_catalog: 'Catálogo',
        source_own: 'Mío',
        source_public: 'Público',
        edit_muscle_group: 'Editar grupo muscular de {{name}}',
        delete_exercise: 'Eliminar ejercicio {{name}}',
        reorder: 'Reordenar {{name}}',
        remove_exercise: 'Eliminar {{name}}',
        source_catalog: 'Catálogo',
        catalog_search: 'Buscar en el catálogo…',
        catalog_hint: 'Escribe para buscar entre miles de ejercicios con guía visual.',
        catalog_empty: 'Sin resultados en el catálogo',
        catalog_loading: 'Cargando catálogo…',
        catalog_error: 'No se pudo cargar el catálogo',
        retry: 'Reintentar',
        target: 'Objetivo',
        secondary: 'Secundarios',
        instructions: 'Instrucciones',
        video: 'Ver vídeo',
        use_in_workout: 'Usar en entreno',
        save_as_own: 'Guardar',
        saved_as_own: 'Guardado en tus ejercicios',
        save_error: 'No se pudo guardar',
      },
      stats: {
        title: 'Estadísticas',
        performance: 'Rendimiento',
        streak_short: 'Racha',
        volume_short: 'Volumen',
        weekly_volume_short: 'Vol. semanal',
        load_error: 'Error al cargar las estadísticas',
        goal_save_error: 'No se pudo guardar el objetivo',
        goal_remove_error: 'No se pudo quitar el objetivo',
        section_training: 'Entrenamiento',
        section_cardio: 'Cardio',
        section_analysis: 'Análisis',
        kpi_vs_last_week: 'Vs. semana pasada',
        kpi_volume: 'volumen',
        kpi_frequency: 'Frecuencia',
        kpi_sessions_30d: 'sesiones (30 días)',
        kpi_avg_duration: 'Duración media',
        kpi_per_session: 'por sesión',
        kpi_total_volume: 'Volumen total',
        kpi_all_time: 'histórico',
        kpi_best_1rm: 'Mejor 1RM',
        kpi_estimated: 'estimado',
        kpi_notes: 'Notas',
        kpi_noted_sets: 'series anotadas',
        kpi_week_sessions: 'Sesiones semana',
        kpi_this_week: 'esta semana',
        kpi_cardio_time: 'Tiempo cardio',
        kpi_distance: 'Distancia',
        kpi_total_sessions: 'Total sesiones',
        kpi_history: 'historial',
        kpi_total_time: 'Tiempo total',
        kpi_total_distance: 'Distancia total',
        cardio_activities: 'Actividades cardio',
        weekly_volume: 'Volumen semanal',
        muscle_distribution: 'Distribución muscular',
        progression_short: 'Progresión',
        best_record: 'Mejor registro',
        goal_1rm: 'Objetivo 1RM',
        compare_exercises: 'Comparar ejercicios',
        rm_calculator: 'Calculadora 1RM',
        reps: 'Reps',
        max_streak: 'Racha máxima',
        estimated_1rm: '1RM estimado',
        rm_unreliable:
          'Por encima de {{reps}} repeticiones la estimación mide más resistencia que fuerza máxima: tómala como orientación.',
        weight_label: 'Peso',
        exercise_a: 'Ejercicio A',
        exercise_b: 'Ejercicio B',
        pick_two: 'Elige dos ejercicios distintos.',
        max_weight: 'Peso máx',
        goal_reached: '¡Objetivo alcanzado! 🎉',
        goal_remaining: 'Faltan {{amount}}',
        goal_placeholder: 'p.ej.',
        goal_set: 'Fijar',
        muscle_dist: 'Distribución Muscular',
        progression: 'Progresión de Fuerza',
        total_volume: 'Volumen Total',
        workouts_done: 'Entrenamientos',
        records: 'Récords',
        no_data: 'Sin datos registrados',
        comparison_title: 'Este mes vs anterior',
        comparison_current: 'Actual',
        comparison_previous: 'Anterior',
        comparison_sessions: 'sesiones',
        projection_title: 'Proyección de volumen',
        projection_next: 'Próxima semana (est.)',
        trend_up: 'Al alza',
        trend_down: 'A la baja',
        trend_flat: 'Estable',
        days: 'días',
        personal_records: 'Records personales',
        total_prs: 'PRs totales',
        possible_stall: 'Posible estancamiento',
        weeks_without_pr_one: '{{count}} semana sin PR',
        weeks_without_pr_other: '{{count}} semanas sin PR',
        stall_tip: 'Prueba subir peso, cambiar el rango de reps o variar el ejercicio.',
        compare_needs_2: 'Necesitas al menos 2 sesiones en común para comparar.',
        chart_needs_2_sessions: 'Necesitas al menos 2 sesiones de {{exercise}}',
        kg_unit: 'kg',
        new_badge: 'Nuevo',
        series_count_one: '{{count}} serie',
        series_count_other: '{{count}} series',
        top_ex_1rm_one: '{{count}} serie · 1RM ~{{value}}{{unit}}',
        top_ex_1rm_other: '{{count}} series · 1RM ~{{value}}{{unit}}',
        kcal_suffix: '{{value}}kcal',
        km_suffix: '{{value}}km',
        cardio_badge: 'Cardio',
        strength_badge: 'Fuerza',
        day_short: 'd',
        total_for_period: 'Total ({{period}})',
      },
      history: {
        title: 'Historial',
        no_workouts: 'Aún no has registrado entrenamientos',
        start_first: 'Empezar primero',
        delete_confirm: '¿Eliminar este registro?',
        sets_view: 'Series',
        workouts_view: 'Entrenos',
        filter_all: 'Todos',
        export_btn: 'Exportar',
        import_btn: 'Importar',
        export_title: 'Exportar datos',
        import_title: 'Importar datos',
        format_excel: 'Excel (.xlsx)',
        format_spreadsheet: 'Excel o CSV',
        format_apple_health: 'Apple Health (peso)',
        health_reading: 'Leyendo el export de Salud…',
        health_zip_hint: 'Descomprime el .zip primero y elige el export.xml que hay dentro.',
        health_invalid: 'Ese fichero no parece un export de Apple Health.',
        health_no_weights: 'El export no trae ningún registro de peso corporal.',
        health_confirm_one: 'Se importará el peso de 1 día.',
        health_confirm_other: 'Se importará el peso de {{count}} días.',
        health_only_gaps:
          'Solo se rellenan los días sin peso: lo que ya tienes apuntado en GymLog no se toca.',
        health_success_one: 'Importado el peso de 1 día.',
        health_success_other: 'Importado el peso de {{count}} días.',
        health_skipped_one: '1 día ya tenía peso y se ha respetado.',
        health_skipped_other: '{{count}} días ya tenían peso y se han respetado.',
        health_all_existing: 'Ya tenías peso en todos esos días; no se ha cambiado nada.',
        format_json: 'JSON',
        repeat: 'Repetir',
        edit: 'Editar',
        edit_title: 'Editar entrenamiento',
        edit_saved: 'Entrenamiento actualizado',
        edit_error: 'No se pudo guardar',
        share: 'Compartir',
        shared_msg: 'Workout compartido',
        save_as_template: 'Plantilla',
        template_prefix: 'Plantilla',
        template_saved: 'Rutina creada desde el entreno',
        import_error: 'Error al importar datos',
        import_success: 'Importados: {{count}}',
        import_excel_success:
          'Excel importado: {{sets}} series, {{cardio}} cardio, {{routines}} rutinas',
        import_confirm_title: 'Confirmar importación',
        import_confirm: 'Importar',
        import_confirm_workouts: 'Se importarán {{workouts}} entrenos y {{sets}} series.',
        import_confirm_excel:
          'El archivo contiene {{sets}} series, {{cardio}} sesiones de cardio y {{routines}} rutinas.',
        import_confirm_duplicates: 'Se saltarán {{count}} series que ya existen en tu historial.',
        import_detected_source: 'Detectado: export de {{app}}.',
        import_skipped_rows_one: '1 fila no es de fuerza (cardio) y no se importará.',
        import_skipped_rows_other: '{{count}} filas no son de fuerza (cardio) y no se importarán.',
        import_duplicates_skipped: '({{count}} duplicadas saltadas)',
        series_plural: 'series',
        search_placeholder: 'Buscar ejercicio o nota…',
        export_json: 'Exportar JSON',
        import_json: 'Importar JSON',
        import_json_invalid: 'JSON no válido: falta "workouts"',
        select_file_login: 'Selecciona un archivo e inicia sesión',
        loading_data: 'Cargando datos...',
        invalid_format: 'Formato no válido',
        file_empty: 'El archivo está vacío',
        file_insufficient: 'El archivo no tiene suficientes datos',
        export_error: 'Error al exportar histórico',
        export_share_title: 'Exportar Historial',
        export_share_dialog: 'Compartir Historial',
        import_none: 'No se pudieron importar datos',
        import_skipped: '({{count}} errores obvios saltados)',
        import_unexpected: 'Error inesperado al importar datos',
        read_file_error: 'Error al leer el archivo desde tu dispositivo',
        cardio_empty: 'Sin sesiones de cardio registradas',
        gym_session: 'Sesión de gimnasio',
        delete_irreversible: 'Esta acción no se puede deshacer.',
        my_stats: 'Mis estadísticas',
        view_label: 'Vista del historial',
        cardio_view: 'Cardio',
        view_all: 'Todo',
        view_cardio: 'Cardio',
        rest_day: 'Descanso',
        unknown_exercise: 'Desconocido',
      },
      auth: {
        login: 'Iniciar sesión',
        signup: 'Registrarse',
        subtitle: 'Semper Fortis',
        email: 'Email',
        password: 'Contraseña',
        name: 'Nombre completo',
        username: 'Nombre de usuario',
        show_password: 'Mostrar contraseña',
        hide_password: 'Ocultar contraseña',
        signin_google: 'Continuar con Google',
        switch_signup: '¿Sin cuenta? Crea una',
        switch_login: '¿Ya tienes cuenta? Inicia sesión',
        loading: 'Cargando…',
        verification: '¡Verifica tu email!',
        download_apk: 'Descargar APK',
        verifying: 'Verificando...',
        error_title: 'Error en la autenticación',
        back_to_login: 'Volver a iniciar sesión',
        processing_error: 'Error al procesar la autenticación',
        error_rate_limited: 'Demasiados intentos. Espera {{seconds}} s',
        error_fill_fields: 'Completa los campos',
        error_password_short: 'La contraseña debe tener al menos 6 caracteres',
        error_fill_profile: 'Completa tu nombre y usuario',
        error_username_short: 'El usuario debe tener al menos 3 caracteres',
        error_password_invalid: 'Contraseña no válida',
        wait_seconds: 'Espera {{seconds}} s',
        or: 'o',
      },
      achievements: {
        title: 'Logros',
        unlocked_toast: '¡Logro desbloqueado!',
        first_workout: 'Primer entreno',
        workouts_10: '10 entrenos',
        workouts_50: '50 entrenos',
        workouts_100: '100 entrenos',
        streak_7: 'Racha de 7 días',
        streak_30: 'Racha de 30 días',
        volume_10t: '10 toneladas',
        volume_100t: '100 toneladas',
        pr_10: '10 récords',
        consistent_month: 'Mes constante',
      },
      permissions: {
        welcome: '¡Bienvenido!',
        intro: 'Para mejorar tu experiencia, puedes activar estas funciones:',
        notifications: 'Notificaciones',
        notifications_desc: 'Fin del descanso, rutina del día y rachas',
        activate: 'Activar',
        thanks: '¡Gracias!',
        continue: 'Continuar',
      },
    },
  },
  en: {
    translation: {
      common: {
        save: 'Save',
        cancel: 'Cancel',
        delete: 'Delete',
        loading: 'Loading...',
        back: 'Back',
        next: 'Next',
        ready: 'Ready',
        close: 'Close',
        confirm: 'Confirm',
        create: 'Create',
        offline: 'Offline — changes will be saved when you reconnect',
        today: 'Today',
        hide: 'Hide',
        show: 'Show',
        remove: 'Remove',
        yesterday: 'Yesterday',
        error_toast: 'Something went wrong',
        error_title: 'Something went wrong',
        error_description: 'Sorry, an unexpected error occurred.',
        reload: 'Reload',
      },
      nav: {
        home: 'Home',
        routines: 'Routines',
        cardio: 'Cardio',
        stats: 'Stats',
        progress: 'Progress',
        settings: 'Settings',
        menu: 'Menu',
        menu_close: 'Close menu',
        group_training: 'Training',
        group_more: 'More',
        streak_days_one: '{{count}} day streak. View stats',
        streak_days_other: '{{count}} day streak. View stats',
      },
      empty: {
        workout_title: 'Start training',
        workout_desc: 'Pick an exercise, add your sets and save your first workout.',
        history_title: 'No history yet',
        history_desc: "Save workouts and they'll show up here for you to review.",
        stats_title: 'No stats yet',
        stats_desc: 'Complete workouts to see your progress and trends here.',
        routine_title: 'No routine set up',
        routine_desc: 'Plan your week by assigning exercises to each day.',
      },
      tips: {
        start_title: 'Start your history',
        start_msg: 'Log at least 3 workouts to get personalized advice based on your own data.',
        streak_strong_title: '{{count}}-day streak!',
        streak_strong_msg:
          'Brutal consistency. Make sure to include at least 1-2 active rest days per week to optimize recovery.',
        streak_good_title: 'Nice streak — {{count}} days',
        streak_good_msg: "You're on track. Keep the rhythm and prioritize sleep to maximize gains.",
        rest_long_title: 'Too long without training',
        rest_long_msg:
          "It's been {{count}} days since your last logged workout. Consistency is the #1 factor for progress. Get back today!",
        rest_mid_title: '{{count}} days without training',
        rest_mid_msg:
          "Resting is normal, but if it's been more than 3 days, plan your next session as soon as possible.",
        freq_low_title: 'Low frequency',
        freq_low_msg:
          'Only {{count}} sessions in the last 30 days. To progress consistently, aim for 3-4 sessions per week.',
        vol_down_title: 'Volume dropped {{pct}}%',
        vol_down_msg:
          'Weekly volume fell significantly. It could be a planned deload (good) or a lack of consistency. Review your plan.',
        vol_up_title: 'Volume rose {{pct}}%',
        vol_up_msg:
          'Big volume increase. Make sure the rise is progressive (max 10-15%/week) to avoid overtraining.',
        vol_progress_title: '+{{pct}}% volume this week',
        vol_progress_msg:
          'Steady volume progression. You keep accumulating training stimulus the right way.',
        balance_push_title: 'Push/pull imbalance',
        balance_push_msg:
          "You're training more push (chest/shoulders) than pull (back/biceps). This can cause postural issues. Increase your back volume.",
        balance_pull_title: 'More pull than push',
        balance_pull_msg:
          'Good emphasis on back and biceps. Consider balancing with chest and shoulder work for complete development.',
        legs_low_title: 'Low leg volume',
        legs_low_msg:
          'Legs make up less than 15% of your volume. They are the largest muscle group — training them more improves anabolic hormones and overall strength.',
        legs_none_title: "You're skipping legs",
        legs_none_msg:
          'No leg exercises logged. Squat, deadlift and leg press are fundamental for overall progress.',
        pr_none_title: 'No recent records',
        pr_none_msg:
          "You haven't set a record in the last 30 days. Try adding 2.5% weight on your main lift or one extra rep.",
        pr_streak_title: '{{count}} new recent records',
        pr_streak_msg:
          "You're on a PR streak! Your progression is excellent. Keep your technique flawless as you add load.",
        dur_short_title: 'Very short sessions',
        dur_short_msg:
          'Averaging {{count}} minutes. Ideal sessions last 45-75 min for optimal stimulus. Add more exercises or sets.',
        dur_long_title: 'Very long sessions',
        dur_long_msg:
          'Averaging {{count}} minutes. Workouts over 2 hours can raise cortisol. Optimize your rest between sets.',
        variety_low_title: 'Low exercise variety',
        variety_low_msg:
          'Only {{count}} distinct exercises. Add variety to stimulate more muscle fibers and avoid adaptation.',
        consistency_title: '{{count}} workouts logged',
        consistency_msg:
          "You've been logging your workouts for a long time! Long-term consistency is the key to success.",
      },
      coach: {
        notify_train_title: 'Time to train',
        notify_train_body: "{{muscle}} hasn't been worked in days. Shall we hit it today?",
        page_title: 'Coach',
        page_subtitle: 'AI analysis of your data',
        generate: 'Analyse my week',
        regenerate: 'Analyse again',
        analyzing: 'Analysing your data...',
        empty_title: 'No analysis yet',
        empty_desc: 'Tap the button and the coach will review your recent weeks.',
        disclaimer:
          'The coach interprets your data, it does not examine you. It does not replace a doctor, a physiotherapist or an in-person trainer.',
        needs_professional_title: 'See a professional',
        needs_professional_body:
          'From what you describe, this is beyond what an app can assess. Get it looked at by a doctor or physiotherapist before training that area again.',
        insights_title: 'What I see',
        suggestions_title: 'What I would do',
        apply: 'Apply',
        dismiss: 'Dismiss',
        applied: 'Applied',
        chat_title: 'Ask the coach',
        chat_empty:
          "Ask anything about your training. It can see your data, so there's no need to explain it.",
        chat_placeholder: 'Type your question...',
        chat_send: 'Send',
        banner_title: 'Coach suggestion',
        banner_dismiss: 'Dismiss suggestion',
        home_hint: 'Review your week and ask away',
        kind: {
          load: 'Load',
          volume: 'Volume',
          frequency: 'Frequency',
          deload: 'Deload',
          rest: 'Rest',
          exercise_swap: 'Exercise swap',
        },
        error: {
          unauthorized: 'Your session expired. Sign in again.',
          consent_required: 'Turn the coach on in Settings to use it.',
          quota_exceeded: "You've used up today's analyses. Come back tomorrow.",
          coach_disabled: 'The coach is unavailable right now.',
          provider_unavailable: 'The service is busy. Try again in a few minutes.',
          invalid_output: 'The response came back malformed. Try again.',
          network: 'No connection. Check your network.',
          unknown: 'Something went wrong. Try again.',
        },
        consent: {
          title: 'Turn on the AI coach',
          intro:
            'The coach sends a summary of your training data to an external AI service to give you personalised advice. It stays off until you turn it on.',
          data_title: 'What leaves your device',
          data_profile: 'Your goal, days per week and available equipment',
          data_body: 'Age band, sex and rounded weight — never your exact date of birth',
          data_training: 'Sets, weights and effort per exercise, already aggregated by week',
          data_wearable: 'If you have a wearable: average sleep and resting heart rate',
          data_never: 'Your name, email, identifier and individual workouts never leave.',
          provider_title: 'Who processes it',
          provider_body:
            'An external AI provider on its free tier. That means there is no data processing agreement like a paid service would have, and they may use requests to improve their services.',
          medical_title: 'What it is not',
          medical_body:
            'It is not medical advice. It does not diagnose, does not prescribe diet or supplements, and does not replace a professional.',
          revoke_title: 'You can back out',
          revoke_body:
            'Turning it off genuinely deletes your coach memory, your conversations and its suggestions.',
          accept: 'Turn on the coach',
          cancel: 'Not now',
        },
        settings: {
          section: 'AI coach',
          toggle: 'AI coach',
          toggle_desc: 'Personalised advice from your data. Off by default.',
          open: 'Open coach',
          memory: 'What it knows about you',
          memory_desc: 'Review and delete what the coach has learned',
          purge: 'Delete coach data',
          purge_title: 'Turn the coach off?',
          purge_confirm:
            "Delete the coach's memory, conversations and suggestions? This cannot be undone.",
          purge_cta: 'Turn off and delete',
          purged: 'Coach data deleted',
        },
        memory: {
          title: 'What it knows about you',
          desc: 'What the coach has learned from your conversations. Delete it whenever you want.',
          empty: "It hasn't learned anything about you yet.",
          delete: 'Delete',
          category: {
            injury: 'Injuries',
            preference: 'Preferences',
            constraint: 'Constraints',
            goal: 'Goals',
          },
        },
        next_session: 'Next session',
        suggestion_title: 'Load suggestion',
        from_to: '{{from}} kg × {{fromReps}} → {{to}} kg × {{toReps}}',
        last_label: 'Last',
        next_label: 'Recommended',
        delta_increase: '+{{deltaKg}} {{unit}} (+{{deltaPct}}%)',
        delta_reduce: '−{{deltaKg}} {{unit}} (−{{deltaPct}}%)',
        delta_hold: '±0 {{unit}}',
        confidence_low: 'Little evidence',
        confidence_medium: 'Some evidence',
        confidence_high: 'Solid evidence',
        volume_weekly_sets_one: '{{count}} set this week',
        volume_weekly_sets_other: '{{count}} sets this week',
        action: {
          increase: 'Add load',
          hold: 'Hold the load',
          reduce: 'Drop the load',
        },
        reason: {
          bodyweight_add_set:
            'You hit the rep ceiling and there is no load to add. Add a set and go back to the bottom of the range.',
          bodyweight_add_load:
            'At this many sets you are training endurance more than strength. Time to add load: start with the smallest plate you can rack.',
          at_failure:
            'You hit the limit and lost reps. Drop 5% and rebuild your technique before adding load again.',
          margin_left: 'You had reps left in the tank in your last sessions. Time to add load.',
          too_hard:
            "You've been pushing past your target for two sessions. Consolidate this weight before adding more.",
          on_target: "You're at the right effort. Keep the weight and try to add one rep.",
          add_rep:
            'The load is too light to add a plate without overshooting. Progress by adding reps.',
          ceiling:
            'You hit the top of your rep range. Add one load step and work your way back up from the bottom.',
          no_effort_reps:
            'No effort recorded (RIR/RPE), so I follow your last session: same weight, one more rep.',
          no_effort_increase:
            'No effort recorded, but you reached the top of the rep range. Bump the load one step.',
          deload:
            'Volume up three weeks running, effort climbing and sessions rated worse. Time for a deload week.',
          no_deload: 'No signs you need a deload yet.',
          finish_the_sets:
            'Only your first set hit the top of the range; the rest fell short. Hold this weight until every set is complete before adding load.',
          stale_data:
            "You haven't trained this lift in over two weeks. Repeat your last weight to see where you stand before adding load.",
          weekly_cap:
            'You already added load this week. Give this weight one more session: two jumps in seven days is more than you can absorb.',
          stall_hold:
            "You've gone several sessions without a new best. More weight won't unstick it: repeat the load and chase rep quality.",
          stall_reset:
            'The plateau is dragging on. Drop 10% and build back up: a running start unsticks more than pushing harder.',
          volume_spike:
            'This muscle is doing far more work this week than in the last four. Hold the load until volume settles.',
          volume_over_mrv:
            'Your weekly sets for this muscle are already past what you usually recover from. The load stays put.',
          volume_below_mev:
            "You're doing very few weekly sets for this muscle. Add sets before touching the weight.",
        },
        readiness: {
          ok: 'Your recovery is on track.',
          low_sleep:
            "You've slept little for several days ({{minutes}} min on average). Hold the load instead of adding to it.",
          high_rhr: 'Your resting heart rate is above your usual average. Hold the load today.',
          sleep_and_hr:
            "You're sleeping little and your resting pulse is high: your body hasn't recovered yet. Hold the load.",
        },
        stall: {
          title: '{{exercise}} has not improved in {{count}} sessions',
          cause_fatigue:
            "You always train at the limit. The problem isn't stimulus, it's recovery: ease off the intensity or rest more between sessions.",
          cause_frequency:
            'Too many days pass between sessions. Training it more often, even with fewer sets, usually unsticks it.',
          cause_volume:
            "You're doing very few sets. Add one or two sets per session before touching the weight.",
          cause_unknown:
            'Try changing something: rep range, tempo, or a variation of the exercise.',
        },
      },
      muscleGroups: {
        pecho: 'Chest',
        espalda: 'Back',
        hombro: 'Shoulders',
        pierna: 'Legs',
        gluteo: 'Glutes',
        biceps: 'Biceps',
        triceps: 'Triceps',
        antebrazo: 'Forearms',
        core: 'Core',
        cardio: 'Cardio',
        otro: 'Other',
      },
      equipment: {
        barra: 'Barbell',
        mancuernas: 'Dumbbells',
        maquina: 'Machine',
        polea: 'Cable',
        peso_corporal: 'Bodyweight',
        bandas: 'Bands',
        kettlebell: 'Kettlebell',
        otro: 'Other',
      },
      muscleMap: {
        title: 'Muscle map',
        front: 'Front',
        back: 'Back',
        legend_recovering: 'Recovering',
        legend_partial: 'Almost ready',
        legend_recovered: 'Recovered',
        legend_untrained: 'Untrained',
        untrained_list: 'No data yet: {{groups}}',
      },
      userStats: {
        page_title: 'My Stats',
        page_subtitle: 'Full analysis of your progress',
        what_now: 'What to do now',
        global_summary: 'Global summary',
        cardio_total_time: 'Total cardio time',
        recovery_rested: 'Recovered',
        recovery_recovering: 'Recovering',
        recovery_partial: 'Almost ready',
        days_ago: '{{count}}d ago',
        no_data_label: 'No data',
        personalized_tips: 'Personalized tips',
        tips_count_one: '{{count}} tip based on your data',
        tips_count_other: '{{count}} tips based on your data',
        empty_title: 'No data yet',
        empty_desc: 'Log your first workouts to see your stats here.',
        calendar_title: 'Workout log',
        calendar_trained_one: '1 day trained',
        calendar_trained_other: '{{count}} days trained',
        weekday_mon: 'M',
        weekday_tue: 'T',
        weekday_wed: 'W',
        weekday_thu: 'T',
        weekday_fri: 'F',
        weekday_sat: 'S',
        weekday_sun: 'S',
        total_workouts: 'Total workouts',
        total_volume: 'Total volume',
        total_prs: 'Personal records',
        streak_days_one: '{{count}} day streak',
        streak_days_other: '{{count}} day streak',
        streak_max_suffix: ' (max. {{max}})',
        sessions_30d: 'sessions/30d',
        avg_duration: 'avg duration',
        distinct_exercises: 'distinct exercises',
        volume_pct: '{{pct}}% volume',
        recovery_title: 'Recovery status',
        recovery_card_title: 'Recovery',
        muscle_recovered: 'Muscle group recovered',
        suggestion_today: 'Suggestion for today',
        train_muscle: 'Train {{muscle}}',
        days_without_training: 'days without training',
        days_since_full: '{{count}} days ago',
        day_frequency_title: 'Consistency by day',
        muscle_balance_title: 'Muscle balance',
        muscle_distribution_label: 'Distribution by muscle group',
        top_exercises_title: 'Top exercises by volume',
        weekly_volume_title: 'Volume evolution',
        last_8_weeks: 'Last 8 weeks',
        favorite_day: 'Your favorite day:',
      },
      onboarding: {
        title: 'Welcome to GymLog v2!',
        subtitle: 'Set up your profile to optimize your experience.',
        goal: 'What is your goal?',
        goal_volume: 'Build muscle',
        goal_strength: 'Gain strength',
        goal_endurance: 'Endurance',
        goal_fat_loss: 'Fat loss',
        days: 'How many days per week?',
        equipment: 'What equipment do you have?',
        finish: 'Start training',
        sex: 'What is your sex?',
        sex_male: 'Male',
        sex_female: 'Female',
        sex_other: 'Rather not say',
        body: 'Your measurements',
        height: 'Height',
        weight: 'Weight',
        body_hint:
          'Used to estimate volume on bodyweight exercises. You can change them any time in My measurements.',
      },
      notifications: {
        title: 'Notifications',
        clear: 'Clear all',
        empty_title: 'No notifications',
        empty_desc: 'Rest alerts, your daily routine and your records will show up here.',
        today: 'Today',
        yesterday: 'Yesterday',
        delete_one: 'Delete this alert',
        filter_all: 'All',
        filter_routine: 'Routine',
        filter_streak: 'Streak',
        filter_summary: 'Summary',
        filter_pr: 'Records',
        empty_filter: 'No alerts of this kind',
        just_now: 'just now',
        minutes_ago: '{{count}} min ago',
        hours_ago: '{{count}} h ago',
        days_ago: '{{count}} d ago',
      },
      search: {
        placeholder: 'Search exercise…',
        clear: 'Clear search',
        empty: 'No exercise matches',
        train: 'Train',
        favorites: 'Favourites',
        favorite_toggle: 'Toggle «{{name}}» as favourite',
        open_library: 'Open the library',
      },
      guide: {
        title: 'How it works',
        subtitle: 'Everything you need to understand GymLog in 2 minutes.',
        quickstart: 'Your first workout in 3 steps',
        step1_title: 'Pick an exercise',
        step1_desc:
          'On the Train tab open the picker and find your exercise. Use "+ Custom" to create your own.',
        step2_title: 'Log your sets',
        step2_desc: 'Type reps and weight, tap "+ Set" and repeat for every set you do.',
        step3_title: 'Save the session',
        step3_desc:
          'Tap Save: the workout goes to your history and updates records, volume and streak.',
        sections_title: 'What each screen does',
        tips_title: 'Good to know',
        open: 'Open',
        finish: 'Got it, let me train!',
        tips: [
          'In history, swipe a workout sideways to delete it.',
          '"Repeat last workout" loads your previous session in one tap.',
          'You can train offline: nothing is lost, it uploads when you are back online.',
          'Everything is reversible: edit or remove a set before saving if you got it wrong.',
        ],
        workout: {
          title: 'Train',
          bullets: [
            'Pick an exercise from the list, or create your own with secondary muscles and a bodyweight option.',
            'Enter reps and weight, tap "+ Set" and keep adding. "× All" clears the list.',
            'Every set supports RPE (perceived effort) and a type: normal, drop set, rest-pause or AMRAP.',
            'Before saving you can add session notes and rate how the workout felt.',
            'If you leave mid-session, the app offers to resume or discard it when you come back.',
          ],
        },
        tools: {
          title: 'In-set helpers',
          bullets: [
            'Plate calculator: give it a target weight and it tells you what to load on each side.',
            'See your estimated 1RM and your records per rep range for each exercise.',
            '"Last session" shows what you did before: copy it or apply the progression suggestion.',
            'On bodyweight exercises, the weight you type counts as added load.',
          ],
        },
        rest: {
          title: 'Rest and alerts',
          bullets: [
            'The timer can start automatically when you add a set (Settings → Training).',
            'Set a fixed duration, or let compound lifts get a longer rest.',
            'The alert fires even if you are on another screen or the app is in the background.',
            'On Android, enable "exact alarms" in Settings so the alert is not delayed.',
          ],
        },
        routines: {
          title: 'Routines',
          bullets: [
            'Plan your week by assigning exercises to each day.',
            'Turn on training reminders and the app will ping you on the right day.',
          ],
        },
        cardio: {
          title: 'Cardio',
          bullets: [
            'Timer for running, cycling, elliptical and other cardio sessions.',
            'The session keeps running if you switch tabs; the Cardio icon shows it is active.',
          ],
        },
        history: {
          title: 'History',
          bullets: [
            'Every saved workout with its sets and volume.',
            'Open any session to review it, or swipe it away to delete it.',
          ],
        },
        stats: {
          title: 'Stats',
          bullets: [
            'Volume per week and per muscle group, personal records and day streak.',
            'Consistency heatmap and muscle fatigue: which groups are recovered and which are not.',
            'Personalized tips based on your goal and your latest sessions.',
          ],
        },
        measurements: {
          title: 'My measurements',
          bullets: [
            'Log bodyweight and measurements to track how they evolve.',
            'Bodyweight is used to estimate volume on bodyweight exercises.',
          ],
        },
        wearables: {
          title: 'Wearables',
          bullets: [
            'Connect Health Connect to pull steps, calories, heart rate and sleep from your watch.',
            'Only works in the Android app, not on the web version.',
          ],
        },
        offline: {
          title: 'Offline',
          bullets: [
            'With no signal the workout is stored on your phone and never lost.',
            'It syncs by itself once you are back online; you will see how many are pending.',
          ],
        },
        settings: {
          title: 'Settings',
          bullets: [
            'Dark or light theme, language, units (kg/lb) and sound.',
            'Notifications, auto rest timer, warm-up sets and biometric unlock.',
          ],
        },
      },
      settings: {
        title: 'Settings',
        theme: 'Theme',
        theme_system: 'System',
        theme_dark: 'Dark',
        accent: 'Accent colour',
        accent_desc: 'Applied to buttons, icons and highlights',
        accent_yellow: 'Yellow',
        accent_lime: 'Lime',
        accent_mint: 'Mint',
        accent_cyan: 'Cyan',
        accent_blue: 'Blue',
        accent_violet: 'Violet',
        accent_pink: 'Pink',
        accent_orange: 'Orange',
        accent_red: 'Red',
        accent_amber: 'Amber',
        accent_green: 'Green',
        accent_teal: 'Teal',
        accent_indigo: 'Indigo',
        accent_magenta: 'Magenta',
        accent_rose: 'Rose',
        accent_sand: 'Sand',
        accent_slate: 'Slate',
        accent_emerald: 'Emerald',
        accent_azure: 'Azure',
        accent_lavender: 'Lavender',
        accent_plum: 'Plum',
        accent_terracotta: 'Terracotta',
        accent_sage: 'Sage',
        accent_steel: 'Steel',
        accent_more: 'Show all colours',
        accent_less: 'Hide colours',
        app_icon: 'App icon',
        app_icon_desc:
          'When you change it, Android removes the icon from your home screen and leaves it at the end of the app drawer. You will have to put it back.',
        app_icon_match: 'Match the accent colour',
        app_icon_changed: 'Icon changed. Look for it in the app drawer.',
        app_icon_error: 'Could not change the icon',
        theme_light: 'Light',
        language: 'Language',
        notifications: 'Notifications',
        notifications_desc: 'Permission for reminders',
        exact_alarms: 'Exact alarms',
        exact_alarms_desc: 'Without this permission the rest alert may arrive late',
        exact_alarms_action: 'Enable',
        exact_alarms_on: 'Exact alarms enabled',
        sound: 'Sound',
        sound_desc: 'Sound on set completion',
        logout: 'Sign out',
        version: 'GymLog v{{version}}',
        about: 'About',
        rest_duration: 'Rest duration',
        rest_duration_desc: 'Auto timer length',
        rest_by_exercise: 'Rest by exercise',
        rest_by_exercise_desc: 'Longer rest on compound lifts (≈2×)',
        auto_fill_weights: 'Auto-fill weights',
        auto_fill_weights_desc: 'Preload last session weight when starting a routine',
        keep_screen_awake: 'Keep screen awake',
        keep_screen_awake_desc:
          'Stops the phone locking while you train. Released when the session ends',
        save_scope: 'Saving sets',
        save_scope_desc: 'What to do when saving with both completed and unmarked sets',
        save_scope_ask: 'Ask',
        save_scope_all: 'Save all',
        save_scope_completed: 'Completed only',
        wearables: 'Devices / Wearables',
        wearables_desc: 'Connect Amazfit, Garmin and more',
        preferences: 'Preferences',
        training: 'Training',
        data: 'Data',
        account: 'Account',
        my_measurements: 'My measurements',
        google_account: 'Google account',
        change_photo: 'Change profile photo',
        photo_updated: 'Profile photo updated',
        photo_error: 'Could not upload the photo',
        photo_too_large: 'Image too large (max 2 MB)',
        edit_name: 'Edit name',
        name_placeholder: 'Your name',
        name_updated: 'Name updated',
        name_error: 'Could not save the name',
        biometric: 'Biometric access',
        biometric_desc: 'Use your fingerprint or face to sign in',
        biometric_unavailable: 'Biometrics unavailable: {{message}}',
        biometric_verifying: 'Verifying identity...',
        biometric_on: 'Biometric access enabled',
        biometric_off: 'Biometric access disabled',
        biometric_failed: 'Authentication failed',
        biometric_sensor_error: 'Could not reach the sensor',
        biometric_locked_title: 'GymLog is locked',
        biometric_locked_hint: 'Use your fingerprint or face to continue',
        biometric_unlock: 'Unlock',
        biometric_retry: 'Try again',
        training_reminders: 'Workout reminders',
        training_reminders_desc: "Alert me about today's routine and my streak at risk",
        reminder_times: 'Reminder times',
        reminder_routine: "Today's routine",
        reminder_streak: 'Streak at risk',
        reminder_summary: 'Weekly summary',
        reminder_summary_day: 'Summary day',
        quiet_hours: 'Do not disturb',
        quiet_hours_desc: 'Inside this range alerts are saved but stay silent',
        quiet_hours_from: 'From',
        quiet_hours_to: 'To',
        quiet_conflict: 'This reminder falls inside the quiet range: it will be saved silently',
        notifications_blocked: 'Android is blocking GymLog notifications',
        notifications_blocked_action:
          'Turn them on in Android Settings → Apps → GymLog → Notifications',
        weight_unit: 'Weight unit',
        warmup_sets: 'Warm-up sets',
        warmup_sets_desc: 'Tag warm-up sets',
        rest_autostart: 'Auto-start rest',
        rest_autostart_desc: 'Starts the timer when you add a set',
        download_apk_title: 'Download Android App',
        tagline: 'Your ultimate companion for strength training tracking.',
        notif_on: 'Notifications enabled',
        notif_denied: 'Notification permission not granted',
        notif_denied_browser: 'Notification permission denied by the browser',
      },
      wearables: {
        title: 'Devices',
        disconnect: 'Disconnect',
        sync_now: 'Sync now',
        syncing: 'Syncing…',
        connected: 'Connected',
        disconnected: 'Not connected',
        last_sync: 'Last sync: {{when}}',
        never_synced: 'Never synced',
        sync_on_open: 'Sync on open',
        sync_on_open_desc: 'Automatically pull data when opening the app',
        health_aggregator: 'Phone health',
        health_aggregator_desc_android: 'Health Connect: Amazfit, Samsung, Garmin and more',
        health_aggregator_desc_ios: 'Apple Health: Amazfit, Apple Watch and more',
        amazfit_hint: 'For Amazfit: enable Zepp app sync to {{target}}.',
        grant_permission: 'Grant permissions',
        permission_needed: 'Grant health permissions to sync',
        steps: 'Steps',
        resting_hr: 'Resting HR',
        distance: 'Distance',
        calories: 'Calories',
        hr_avg_max: 'HR avg/max',
        sleep: 'Sleep',
        sleep_duration: 'Sleep duration',
        sync_ok: 'Synced: {{daily}} days, {{sleep}} nights, {{workouts}} workouts',
        sync_error: 'Sync failed',
        not_available_web: 'Only available in the mobile app',
        no_data: 'No data yet. Sync to get started.',
        bpm: 'bpm',
        strength_imported_title: 'Gym sessions',
        strength_imported_desc:
          "{{count}} gym sessions were imported with their time, calories and heart rate. They don't include sets or weights — you log those in the app, and both show up together in your history.",
      },
      workout: {
        title: 'Train',
        select_exercise: '- Exercise -',
        custom_exercise: '+ Custom',
        notes: 'Notes',
        no_notes: 'Annotations',
        new_note: 'New note...',
        recent_pr: 'Recent PR',
        pr_by_band: 'Records by rep range',
        sets: 'Sets',
        volume: 'Volume',
        active_session: 'Active session',
        repeat_last: 'Repeat last workout',
        start_cta: 'Start training',
        cancel_session: 'Cancel workout',
        cancel_confirm: 'Cancel and delete the current workout session?',
        cancel_confirm_title: 'Discard this workout?',
        cancel_confirm_body_one: 'The set you logged will be deleted. This cannot be undone.',
        cancel_confirm_body_other:
          'The {{count}} sets you logged will be deleted. This cannot be undone.',
        cancel_confirm_accept: 'Discard',
        cancel_confirm_keep: 'Keep training',
        reps: 'Reps',
        seconds: 'Seconds',
        mode_reps: 'Reps',
        mode_time: 'Time',
        timer_start: 'Start',
        timer_pause: 'Pause',
        timer_reset: 'Reset',
        timer_accept: 'Accept the time',
        timer_elapsed: '{{seconds}} seconds',
        timer_target: 'Target {{time}}',
        weight: 'Kg',
        add_set: '+ Set',
        complete_set: 'Complete set',
        previous_set: 'Previous',
        save_pending_title: 'Save workout',
        save_pending_body:
          'You have {{pending}} unchecked sets and {{completed}} completed. Save them all or only the completed ones?',
        save_all: 'Save all',
        save_completed_only: 'Completed only',
        set_n: 'Set {{n}}',
        edit_set: 'Edit set',
        warmup: 'Warm-up',
        set_note_placeholder: 'Set note...',
        remove_all: '× All',
        rest: 'Rest',
        rest_auto_on: 'Auto on',
        rest_auto_off: 'Auto off',
        rest_remaining: 'Rest remaining',
        rest_restart: 'Restart rest',
        rest_over: 'Rest is over!',
        stop_alarm: 'Stop',
        save_workout: 'Save',
        saving: 'Saving...',
        session_notes: 'Session notes',
        session_notes_placeholder: 'How did it feel? Sleep, energy...',
        session_rating: 'Rating',
        empty_sets: 'Add a set',
        empty_hint: 'Pick an exercise and log your sets.',
        new_pr_title: '🏆 New Personal Record!',
        new_pr_body: '{{exercise}}: {{weight}} kg estimated 1RM',
        resume_banner: 'Workout in progress detected',
        resume_desc: 'You have an active session. Want to continue?',
        continue: 'Continue',
        discard: 'Discard',
        add_valid_set: 'Add at least one valid set',
        errors: {
          needs_measure: 'Add reps or seconds',
          weight_negative: 'Weight cannot be negative',
          invalid_set: 'Incomplete set',
          pick_exercise: 'Pick an exercise before saving',
          no_valid_sets: 'Add valid reps and weight',
          sets_incomplete_one: 'Set {{numbers}} is unfinished',
          sets_incomplete_other: 'Sets {{numbers}} are unfinished',
        },
        saved: '✓ Workout saved',
        saved_offline: '✓ Saved offline · will sync',
        pending_sync_one: '1 workout pending sync',
        pending_sync_other: '{{count}} workouts pending sync',
        note_save_error: 'Could not save the note',
        confirm_delete_exercise: 'Delete this exercise? This action cannot be undone.',
        delete_exercise_title: 'Delete “{{name}}”?',
        delete_exercise_body:
          'It disappears from your list and its personal records are lost. This cannot be undone.',
        delete_exercise_in_use: 'Cannot delete: you have saved workouts using this exercise.',
        delete_exercise_failed: 'Could not delete the exercise.',
        delete_exercise_done: 'Exercise deleted',
        copy_replace_title: 'Replace the sets you logged?',
        copy_replace_body:
          'Copying the last session replaces what you logged for this exercise. This cannot be undone.',
        copy_replace_accept: 'Replace',
        custom_badge: 'Custom',
        create_custom_exercise: 'Create custom exercise',
        new_exercise: 'New exercise',
        exercise_name_placeholder: 'Exercise name',
        secondary_muscles: 'Secondary muscles',
        group_recent: 'Recent',
        group_suggested: '{{group}} — Suggested',
        bodyweight_exercise: 'Bodyweight exercise',
        name_required: 'Name is required',
        bodyweight_hint: 'Body weight: {{weight}}. Any kg you enter is added as external load.',
        bodyweight_no_weight:
          'Record your weight (above) to estimate volume. You can save with reps only.',
        load_label: 'Added load',
        load_type_label: 'How it is done',
        load_type_prompt: 'How do you do “{{name}}”?',
        load_type_external: 'External',
        load_type_bodyweight: 'Bodyweight',
        load_type_bodyweight_loaded: 'Weighted',
        rpe: 'RPE',
        rpe_label: 'Rate of perceived exertion (RPE)',
        rpe_optional: '· optional',
        rpe_option: 'RPE {{value}}',
        rpe_help:
          'From 6 (very easy) to 10 (no more reps in you). Logging it is what lets the app tune your load and tell you when to deload.',
        context_hide: 'Hide context',
        context_show: 'Show last session',
        set_type_label: 'Set type',
        set_type_normal: 'Normal',
        set_type_dropset: 'Drop set',
        set_type_rest_pause: 'Rest-pause',
        set_type_amrap: 'AMRAP',
        plates_calc: 'Plate calculator',
        plates_calc_short: 'Plate calc.',
        rate_session: 'Rate session',
        change: 'Change',
        set_note: 'Set note',
        plates_target: 'Target weight',
        plates_bar: 'Bar',
        plates_each_side: 'On each side of the bar',
        plates_empty: 'Bar only',
        plates_leftover: 'Not exact: {{kg}} kg left per side',
        plates_available: 'Plates at my gym',
        plates_available_none: 'none',
        plates_available_help: 'Tick the plates you have. They are remembered for next time.',
        equipment_steps: 'Dumbbell and machine steps',
        equipment_steps_help:
          'Plates only describe the barbell. These are the smallest jumps on the rest of the equipment, so load suggestions stop proposing weights you cannot load.',
        dumbbell_step: 'Dumbbell (kg)',
        machine_step: 'Machine (kg)',
        e1rm: 'Est. 1RM',
        last_session: 'Last session',
        copy: 'Copy',
        progression_hint: 'Suggestion',
        apply: 'Apply',
        complete_title: 'Workout saved',
        complete_minutes: '{{count}} min',
        complete_sets: '{{count}} sets',
      },
      cardio: {
        no_records: 'No records',
        recording: 'Recording',
        paused: 'Paused',
        in_progress: 'In progress',
        resume: 'Resume',
        pause: 'Pause',
        finish: 'Finish',
        add_details: 'Add optional details',
        distance_km: 'Distance (km)',
        calories: 'Calories',
        notes_placeholder: 'Notes (optional)...',
        discard: 'Discard',
        save_session: 'Save session',
        this_week: 'This week',
        sessions: 'Sessions',
        time: 'Time',
        distance: 'Distance',
        recent_activity: 'Recent activity',
        pick_activity: 'Start an activity',
        pick_activity_help: 'Tap one and the timer starts',
        start_type: 'Start {{type}}',
        start_session: 'Start session',
        first_session: 'Start your first cardio session',
        health_source: 'Health',
      },
      routine: {
        title: 'Routines',
        select: 'Pick a routine',
        change: 'Change',
        today: 'TODAY',
        predefined: 'Preset',
        use_template: 'Use as template',
        cloned: 'Template copied — you can edit it now',
        create_custom: '+ Create custom routine',
        share: 'Share routine',
        share_dialog_title: 'Share routine',
        share_error: "Couldn't share the routine.",
        print: 'Print or save as PDF',
        print_dialog_title: 'Open the routine sheet',
        print_error: "Couldn't prepare the sheet.",
        print_popup_blocked: 'The browser blocked the window. Allow pop-ups and try again.',
        import_file: 'Import routine from a file',
        import_success: 'Routine "{{name}}" added.',
        import_error: "Couldn't read the routine.",
        new_routine: 'New Routine',
        name_placeholder: 'Name',
        desc_placeholder: 'Description (optional)',
        custom_default_desc: 'Custom routine',
        delete_routine: 'Delete routine',
        delete_confirm: 'Delete this routine?',
        delete_confirm_desc: 'The routine and its days go. Workouts you already saved stay.',
        empty_custom_day: 'This day is empty',
        empty_custom_day_desc: 'Add the exercises you want to do on this day.',
        rest_day: 'Rest day',
        rest_day_desc: 'Nothing today. Resting is part of the plan too.',
        starts_on: 'Starts {{date}}',
        schedule_short: 'Schedule',
        schedule_action: 'Schedule when {{name}} starts',
        hide_short: 'Hide',
        hide_action: 'Hide the {{name}} routine',
        unhide_short: 'Show',
        unhide_action: 'Show the {{name}} routine again',
        hidden_show_one: 'Show 1 hidden routine',
        hidden_show_other: 'Show {{count}} hidden routines',
        hidden_collapse: 'Show less',
        hidden_hint: 'Hidden routines are not deleted: they stay here in case you want them back.',
        keep_current: 'Keep my current routine',
        hidden_done: '{{name}} no longer shows in the list',
        hidden_undo: 'Undo',
        hidden_scheduled: 'Scheduled',
        schedule_title: 'Schedule this routine',
        schedule_help:
          'On the day you pick, this routine becomes the active one by itself. If you do not open the app that day, it kicks in next time you do.',
        schedule_date_label: 'Starts on',
        schedule_saved: 'Scheduled for {{date}}',
        schedule_cleared: 'Date removed',
        schedule_clear: 'Remove date',
        schedule_applied: 'New block starts: {{name}}',
        next_scheduled: '“{{name}}” starts {{date}}',
        move_day: 'Move',
        move_title: 'Move {{name}} to…',
        move_help: 'This only changes the current week. Monday your usual routine is back.',
        move_free: 'Free',
        move_done: 'Moved to {{day}}',
        week_reorganized: 'Week rearranged',
        week_reorganized_desc: 'These changes only apply to this week.',
        week_restore: 'Restore week',
        week_restored: 'Week restored',
        moved_from: 'Moved from {{day}}',
        day_freed: 'Free day',
        day_freed_desc: 'You moved this session to another day this week.',
        add_exercise: '+ Add',
        add_exercise_full: 'Add exercise',
        edit_exercise: 'Edit {{name}}',
        exercise_mode: 'How it is done',
        mode_reps: 'Reps',
        mode_time: 'Time',
        field_sets: 'Sets',
        field_reps: 'Reps',
        field_reps_placeholder: '10-12',
        field_duration: 'Seconds',
        per_side: 'Per side',
        per_side_help: 'The target is per side: "3 x 12 per side" is 24 reps.',
        target_per_side: 'per side',
        superset: 'Superset',
        superset_help: 'Chained with "{{name}}", no rest between the two.',
        superset_label: 'Superset',
        save_failed: 'Could not save the routine. Please try again.',
        start_session: 'Do this routine',
        session_in_progress: 'Routine in progress',
        session_target: 'Target',
        session_finish: 'Complete routine',
        session_discard: 'Discard',
        session_discard_confirm: 'Discard the routine in progress? Your sets will be lost.',
        session_saving: 'Saving…',
        session_saved: 'Routine logged to your history',
        session_saved_offline: 'Routine saved — it will sync when you are back online',
        days: {
          monday: 'Monday',
          tuesday: 'Tuesday',
          wednesday: 'Wednesday',
          thursday: 'Thursday',
          friday: 'Friday',
          saturday: 'Saturday',
          sunday: 'Sunday',
        },
        session_dropped_sets_one: 'One set was left unfinished and was not saved',
        session_dropped_sets_other: '{{count}} sets were left unfinished and were not saved',
        errors: {
          session_no_sets: 'Log at least one set with reps and weight',
        },
        session_empty: 'Log at least one set with reps and weight',
        session_how_to: 'How to do it',
        session_recommended_weight: 'Recommended weight',
        session_no_recommendation:
          'Not enough history to recommend a weight yet. Type the sets yourself and next time you get a load.',
        session_no_advice:
          'No set has a weight or a time yet. Fill in at least one before finishing.',
        session_log: 'What you actually did',
        session_add_set: 'Add set',
        session_remove_set: 'Remove set {{n}}',
        session_reps_of_set: 'Reps of set {{n}}',
        session_weight_of_set: 'Weight of set {{n}}',
      },
      measurements: {
        title: 'Body measurements',
        weight: 'Weight',
        body_fat: 'Body fat',
        add: 'Add measurement',
        empty: 'No measurements yet',
        save_error: 'Could not save measurement',
      },
      weight_prompt: {
        title: 'What do you weigh this week?',
        subtitle: 'We use it to estimate the volume of bodyweight exercises.',
        placeholder: 'kg',
        save: 'Save',
        invalid: 'Enter a weight between 1 and {{max}} kg',
      },
      library: {
        title: 'Exercise library',
        open: 'Library',
        search: 'Search exercise…',
        empty: 'No exercises',
        muscle: 'Muscle',
        equipment: 'Equipment',
        movement: 'Movement',
        compound: 'Compound',
        no_description: 'No instructions yet',
        all: 'All',
        all_equipment: 'All equipment',
        tab_own: 'My exercises',
        tab_catalog: 'Catalog',
        source_own: 'Mine',
        source_public: 'Public',
        edit_muscle_group: 'Edit muscle group for {{name}}',
        delete_exercise: 'Delete exercise {{name}}',
        reorder: 'Reorder {{name}}',
        remove_exercise: 'Remove {{name}}',
        source_catalog: 'Catalog',
        catalog_search: 'Search the catalog…',
        catalog_hint: 'Type to search thousands of exercises with visual guidance.',
        catalog_empty: 'No catalog results',
        catalog_loading: 'Loading catalog…',
        catalog_error: 'Could not load the catalog',
        retry: 'Retry',
        target: 'Target',
        secondary: 'Secondary',
        instructions: 'Instructions',
        video: 'Watch video',
        use_in_workout: 'Use in workout',
        save_as_own: 'Save',
        saved_as_own: 'Saved to your exercises',
        save_error: 'Could not save',
      },
      stats: {
        title: 'Statistics',
        performance: 'Performance',
        streak_short: 'Streak',
        volume_short: 'Volume',
        weekly_volume_short: 'Weekly vol.',
        load_error: 'Could not load statistics',
        goal_save_error: 'Could not save the goal',
        goal_remove_error: 'Could not remove the goal',
        section_training: 'Training',
        section_cardio: 'Cardio',
        section_analysis: 'Analysis',
        kpi_vs_last_week: 'Vs. last week',
        kpi_volume: 'volume',
        kpi_frequency: 'Frequency',
        kpi_sessions_30d: 'sessions (30 days)',
        kpi_avg_duration: 'Average duration',
        kpi_per_session: 'per session',
        kpi_total_volume: 'Total volume',
        kpi_all_time: 'all time',
        kpi_best_1rm: 'Best 1RM',
        kpi_estimated: 'estimated',
        kpi_notes: 'Notes',
        kpi_noted_sets: 'annotated sets',
        kpi_week_sessions: 'Sessions this week',
        kpi_this_week: 'this week',
        kpi_cardio_time: 'Cardio time',
        kpi_distance: 'Distance',
        kpi_total_sessions: 'Total sessions',
        kpi_history: 'history',
        kpi_total_time: 'Total time',
        kpi_total_distance: 'Total distance',
        cardio_activities: 'Cardio activities',
        weekly_volume: 'Weekly volume',
        muscle_distribution: 'Muscle distribution',
        progression_short: 'Progression',
        best_record: 'Best record',
        goal_1rm: '1RM goal',
        compare_exercises: 'Compare exercises',
        rm_calculator: '1RM calculator',
        reps: 'Reps',
        max_streak: 'Max streak',
        estimated_1rm: 'Estimated 1RM',
        rm_unreliable:
          'Above {{reps}} reps the estimate says more about endurance than maximal strength — treat it as a ballpark.',
        weight_label: 'Weight',
        exercise_a: 'Exercise A',
        exercise_b: 'Exercise B',
        pick_two: 'Pick two different exercises.',
        max_weight: 'Max weight',
        goal_reached: 'Goal reached! 🎉',
        goal_remaining: '{{amount}} to go',
        goal_placeholder: 'e.g.',
        goal_set: 'Set',
        muscle_dist: 'Muscle Distribution',
        progression: 'Strength Progression',
        total_volume: 'Total Volume',
        workouts_done: 'Workouts',
        records: 'Records',
        no_data: 'No data recorded',
        comparison_title: 'This month vs previous',
        comparison_current: 'Current',
        comparison_previous: 'Previous',
        comparison_sessions: 'sessions',
        projection_title: 'Volume projection',
        projection_next: 'Next week (est.)',
        trend_up: 'Trending up',
        trend_down: 'Trending down',
        trend_flat: 'Stable',
        days: 'days',
        personal_records: 'Personal records',
        total_prs: 'Total PRs',
        possible_stall: 'Possible stagnation',
        weeks_without_pr_one: '{{count}} week without a PR',
        weeks_without_pr_other: '{{count}} weeks without a PR',
        stall_tip:
          'Try increasing the weight, changing the rep range, or switching up the exercise.',
        compare_needs_2: 'You need at least 2 sessions in common to compare.',
        chart_needs_2_sessions: 'You need at least 2 sessions of {{exercise}}',
        kg_unit: 'kg',
        new_badge: 'New',
        series_count_one: '{{count}} set',
        series_count_other: '{{count}} sets',
        top_ex_1rm_one: '{{count}} set · 1RM ~{{value}}{{unit}}',
        top_ex_1rm_other: '{{count}} sets · 1RM ~{{value}}{{unit}}',
        kcal_suffix: '{{value}}kcal',
        km_suffix: '{{value}}km',
        cardio_badge: 'Cardio',
        strength_badge: 'Strength',
        day_short: 'd',
        total_for_period: 'Total ({{period}})',
      },
      history: {
        title: 'History',
        no_workouts: 'No workouts recorded yet',
        start_first: 'Start first',
        delete_confirm: 'Delete this record?',
        sets_view: 'Sets',
        workouts_view: 'Workouts',
        filter_all: 'All',
        export_btn: 'Export',
        import_btn: 'Import',
        export_title: 'Export data',
        import_title: 'Import data',
        format_excel: 'Excel (.xlsx)',
        format_spreadsheet: 'Excel or CSV',
        format_apple_health: 'Apple Health (weight)',
        health_reading: 'Reading the Health export…',
        health_zip_hint: 'Unzip the .zip first and pick the export.xml inside.',
        health_invalid: "That file doesn't look like an Apple Health export.",
        health_no_weights: 'The export has no body-weight records.',
        health_confirm_one: '1 day of body weight will be imported.',
        health_confirm_other: '{{count}} days of body weight will be imported.',
        health_only_gaps:
          'Only days without a weight are filled in — what you already logged in GymLog is left alone.',
        health_success_one: 'Imported 1 day of body weight.',
        health_success_other: 'Imported {{count}} days of body weight.',
        health_skipped_one: '1 day already had a weight and was kept.',
        health_skipped_other: '{{count}} days already had a weight and were kept.',
        health_all_existing: 'You already had a weight on all those days; nothing changed.',
        format_json: 'JSON',
        repeat: 'Repeat',
        edit: 'Edit',
        edit_title: 'Edit workout',
        edit_saved: 'Workout updated',
        edit_error: 'Could not save',
        share: 'Share',
        shared_msg: 'Workout shared',
        save_as_template: 'Template',
        template_prefix: 'Template',
        template_saved: 'Routine created from workout',
        import_error: 'Error importing data',
        import_success: 'Imported: {{count}}',
        import_excel_success:
          'Excel imported: {{sets}} sets, {{cardio}} cardio, {{routines}} routines',
        import_confirm_title: 'Confirm import',
        import_confirm: 'Import',
        import_confirm_workouts: '{{workouts}} workouts and {{sets}} sets will be imported.',
        import_confirm_excel:
          'The file contains {{sets}} sets, {{cardio}} cardio sessions and {{routines}} routines.',
        import_confirm_duplicates: '{{count}} sets already in your history will be skipped.',
        import_detected_source: 'Detected: {{app}} export.',
        import_skipped_rows_one: '1 row is not a strength set (cardio) and will be skipped.',
        import_skipped_rows_other:
          '{{count}} rows are not strength sets (cardio) and will be skipped.',
        import_duplicates_skipped: '({{count}} duplicates skipped)',
        series_plural: 'sets',
        search_placeholder: 'Search exercise or note…',
        export_json: 'Export JSON',
        import_json: 'Import JSON',
        import_json_invalid: 'Invalid JSON: missing "workouts"',
        select_file_login: 'Select a file and sign in',
        loading_data: 'Loading data...',
        invalid_format: 'Invalid format',
        file_empty: 'The file is empty',
        file_insufficient: 'The file does not have enough data',
        export_error: 'Error exporting history',
        export_share_title: 'Export History',
        export_share_dialog: 'Share History',
        import_none: 'No data could be imported',
        import_skipped: '({{count}} obvious errors skipped)',
        import_unexpected: 'Unexpected error importing data',
        read_file_error: 'Error reading the file from your device',
        cardio_empty: 'No cardio sessions logged',
        gym_session: 'Gym session',
        delete_irreversible: 'This action cannot be undone.',
        my_stats: 'My stats',
        view_label: 'History view',
        cardio_view: 'Cardio',
        view_all: 'All',
        view_cardio: 'Cardio',
        rest_day: 'Rest',
        unknown_exercise: 'Unknown',
      },
      auth: {
        login: 'Login',
        signup: 'Sign up',
        subtitle: 'Your ultimate training companion',
        email: 'Email',
        password: 'Password',
        name: 'Full Name',
        username: 'Username',
        show_password: 'Show password',
        hide_password: 'Hide password',
        signin_google: 'Continue with Google',
        switch_signup: 'No account? Create one',
        switch_login: 'Already have an account? Login',
        loading: 'Loading…',
        verification: 'Check your email!',
        download_apk: 'Download APK',
        verifying: 'Verifying...',
        error_title: 'Authentication error',
        back_to_login: 'Back to sign in',
        processing_error: 'Error processing authentication',
        error_rate_limited: 'Too many attempts. Wait {{seconds}}s',
        error_fill_fields: 'Fill in the fields',
        error_password_short: 'Password must be at least 6 characters',
        error_fill_profile: 'Fill in your name and username',
        error_username_short: 'Username must be at least 3 characters',
        error_password_invalid: 'Invalid password',
        wait_seconds: 'Wait {{seconds}}s',
        or: 'or',
      },
      achievements: {
        title: 'Achievements',
        unlocked_toast: 'Achievement unlocked!',
        first_workout: 'First workout',
        workouts_10: '10 workouts',
        workouts_50: '50 workouts',
        workouts_100: '100 workouts',
        streak_7: '7-day streak',
        streak_30: '30-day streak',
        volume_10t: '10 tonnes',
        volume_100t: '100 tonnes',
        pr_10: '10 records',
        consistent_month: 'Consistent month',
      },
      permissions: {
        welcome: 'Welcome!',
        intro: 'To improve your experience, you can enable these features:',
        notifications: 'Notifications',
        notifications_desc: 'Rest alerts, your daily routine and your streaks',
        activate: 'Enable',
        thanks: 'Thanks!',
        continue: 'Continue',
      },
    },
  },
};
